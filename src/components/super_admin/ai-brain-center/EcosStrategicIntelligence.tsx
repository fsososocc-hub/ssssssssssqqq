...
import { autonomousPlanningService } from '../../../services/AutonomousPlanningService';
import { PlanningGoalItem, PlanningTaskItem, PlanningRunItem } from '../../../types';
...
export default function EcosStrategicIntelligence() {
  ...
  // Autonomous planning states
  const [detectedOpps, setDetectedOpps] = useState<any[]>([]);
  const [planningGoals, setPlanningGoals] = useState<PlanningGoalItem[]>([]);
  const [planningTasks, setPlanningTasks] = useState<PlanningTaskItem[]>([]);
  const [planningRuns, setPlanningRuns] = useState<PlanningRunItem[]>([]);
  const [selectedGoalId, setSelectedGoalId] = useState<string | null>(null);
  const [planAnalysisStatus, setPlanAnalysisStatus] = useState<string>('');

  // Selected Active Sub-view in Strategic Workspace
  const [subTab, setSubTab] = useState<'core' | 'objectives' | 'scenarios' | 'tradeoffs' | 'board' | 'capital' | 'portfolio' | 'fashion_intel' | 'planning_runtime'>('core');
  ...
  const reloadData = () => {
    ...
    // Autonomous planning reloads
    setDetectedOpps(autonomousPlanningService.detectOpportunities(tenantId));
    setPlanningGoals(dbEngine.planning_goals.getAll().filter(g => g.merchant_id === tenantId));
    setPlanningTasks(dbEngine.planning_tasks.getAll());
    setPlanningRuns(dbEngine.planning_runs.getAll());
  };
  ...
  return (
    <div className="bg-slate-50 border border-slate-200/80 rounded-xl overflow-hidden shadow-sm" id="strategic_intelligence_master">
      ...
      <div className="bg-slate-100 border-b border-slate-200 px-4 py-2 text-xs font-semibold flex flex-wrap gap-1.5">
        ...
        <button 
          onClick={() => setSubTab('planning_runtime')}
          className={`px-3 py-1.5 rounded transition ${subTab === 'planning_runtime' ? 'bg-[#07C2E3] text-black font-extrabold border border-[#07C2E3]' : 'text-slate-700 hover:bg-slate-200 border border-slate-300'}`}
        >
          🔮 P0-011 主动规划运行时 (Autonomous Planning Engine)
        </button>
      </div>
      ...
      <div className="p-4 md:p-6" id="strategic_content_body">
        ...
        {/* P0-011: AUTONOMOUS PLANNING RUNTIME UI */}
        {subTab === 'planning_runtime' && (
          <div className="space-y-6 text-slate-100 font-sans" id="autonomous_planning_runtime_panel">
            {/* Header */}
            <div className="bg-slate-950 border border-slate-800 p-6 rounded-2xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="px-2 py-0.5 rounded text-[10px] bg-[#07C2E3]/20 text-[#07C2E3] font-bold tracking-wider uppercase border border-[#07C2E3]/35">P0-011</span>
                  <h4 className="text-white text-base font-black tracking-wide uppercase">ECOS 主动式经营规划内核</h4>
                </div>
                <p className="text-xs text-slate-400">系统进入主动运营范式，基于 World State 主动预测风险，编排 14 个 Agent 执行任务并自我闭环</p>
              </div>
              <button
                onClick={() => {
                  setPlanAnalysisStatus('正在巡检全网商业指标...');
                  setTimeout(() => {
                    const generated = autonomousPlanningService.generateGoalsFromOpportunities(tenantId);
                    setPlanAnalysisStatus(`✓ 巡检完毕。成功检测到并产生 ${generated.length} 个新战略运营目标！已存入数据库。`);
                    reloadData();
                  }, 1200);
                }}
                className="px-4 py-2 bg-[#07C2E3] text-black hover:bg-[#06B2D0] font-black rounded text-xs flex items-center gap-1.5 transition uppercase tracking-wider shadow-[0_0_15px_rgba(7,194,227,0.25)]"
              >
                <Sparkles className="w-4 h-4" /> 自动扫描并生成战略目标
              </button>
            </div>

            {planAnalysisStatus && (
              <div className="p-3 bg-slate-950/80 border border-slate-800 rounded-lg text-xs flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-[#07C2E3] animate-ping" />
                <span className="font-mono text-[#07C2E3]">{planAnalysisStatus}</span>
              </div>
            )}

            {/* Top Grid: Opportunity Detection vs Goal Setting */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              
              {/* Opportunity Detection Engine Output */}
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
                <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h5 className="text-xs font-black text-white uppercase tracking-wider">Opportunity Detection Engine</h5>
                      <span className="text-[10px] text-slate-500">动态商业机会与风险扫描输出</span>
                    </div>
                  </div>
                  <span className="text-xs font-mono text-slate-400 font-bold bg-slate-955 px-2 py-0.5 rounded border border-slate-800">
                    {detectedOpps.length} 个待办信号
                  </span>
                </div>

                <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
                  {detectedOpps.map((opp, idx) => (
                    <div key={idx} className="p-3.5 bg-slate-950 border border-slate-800/80 rounded-xl space-y-2 hover:border-[#07C2E3]/35 transition">
                      <div className="flex items-center justify-between">
                        <span className={`text-[9px] px-2 py-0.5 rounded font-black uppercase ${
                          opp.severity === 'critical' ? 'bg-rose-500/20 text-rose-400 border border-rose-500/30' : 
                          opp.severity === 'warning' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30' : 
                          'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30'
                        }`}>
                          {opp.type} | {opp.severity}
                        </span>
                        <span className="font-mono text-[10px] text-[#07C2E3] font-bold">{opp.potentialGain}</span>
                      </div>
                      <h6 className="text-xs font-bold text-white leading-snug">{opp.title}</h6>
              ...
              {/* Goal Setting Engine (planning_goals) */}
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-4">
                <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                  <div className="flex items-center gap-2">
                    <div className="p-1.5 rounded-lg bg-[#07C2E3]/10 text-[#07C2E3]">
                      <Target className="w-4 h-4" />
                    </div>
                    <div>
                      <h5 className="text-xs font-black text-white uppercase tracking-wider">Active Strategic Objectives</h5>
                      <span className="text-[10px] text-slate-500">持久化落库战略规划目标（planning_goals）</span>
                    </div>
                  </div>
                  <span className="text-xs font-mono text-slate-400 font-bold bg-slate-955 px-2 py-0.5 rounded border border-slate-800">
                    {planningGoals.length} 项记录
                  </span>
                </div>
                ...
                <div className="space-y-3 max-h-[300px] overflow-y-auto pr-1">
                  {planningGoals.length === 0 ? (
                    <div className="py-12 text-center text-slate-500 text-xs">
                      当前平台暂无主动生成的战略目标。<br />点击右上角「自动扫描并生成目标」重新派发。
                    </div>
                  ) : (
                    planningGoals.map((goal) => (
                      <div 
                        ...
                        className={`p-3.5 border rounded-xl space-y-2 cursor-pointer transition ${
                          selectedGoalId === goal.goal_id ? 'bg-[#07C2E3]/5 border-[#07C2E3]' : 'bg-slate-950 border-slate-800 hover:border-slate-700'
                        }`}
                      >
                        <div className="flex items-center justify-between">
                          <span className="text-[9px] font-mono font-bold text-[#07C2E3]">{goal.goal_id}</span>
                          <span className={`text-[9px] px-2 py-0.5 rounded font-black uppercase ${
                            goal.status === 'completed' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' :
                            goal.status === 'active' ? 'bg-sky-500/20 text-sky-400 border border-sky-500/30' :
                            goal.status === 'planning' ? 'bg-yellow-500/20 text-yellow-400 border border-yellow-500/30' :
                            'bg-slate-800 text-slate-400'
                          }`}>
                            {goal.status}
                          </span>
                        </div>
                        <h6 className="text-xs font-bold text-white leading-snug flex items-center gap-1.5">
                          {goal.goal_name}
                        </h6>
                        <div className="pt-2 border-t border-slate-900 flex items-center justify-between text-[11px]">
                          <span className="text-slate-500">优先级: <strong className="text-slate-300">#{goal.priority}</strong></span>
                          <span className="font-mono font-bold text-[#07C2E3]">预期价值: {goal.target_value}</span>
                        </div>
                      </div>
                    ))
                  )}
            ...
            {/* Bottom Section: Plan Details, Simulation and Action Decomposition */}
            {selectedGoalId && (() => {
              const selectedGoal = planningGoals.find(g => g.goal_id === selectedGoalId);
              const relatedTasks = planningTasks.filter(t => t.goal_id === selectedGoalId);
              const relatedRun = planningRuns.find(r => r.goal_id === selectedGoalId);
              ...
              return (
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 space-y-6 animate-fadeIn">
                  <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-slate-800 pb-4">
                    <div>
                      <h4 className="text-white text-sm font-black tracking-wider uppercase">
                        战略执行详情 & 协作编排阵列 [目标: {selectedGoal.goal_name}]
                      </h4>
                      <p className="text-xs text-slate-500 mt-0.5">
                        基于 <strong>DNA 经营治理协议</strong> 智能分解给相关岗位的多层任务执行流
                      </p>
                    </div>
                    <div className="flex gap-2">
                      <button
                        onClick={() => {
                          setPlanAnalysisStatus(`正在为战略ID [${selectedGoalId}] 生成原子任务分解...`);
                          setTimeout(() => {
                            autonomousPlanningService.generatePlanningPipeline(selectedGoalId);
                            setPlanAnalysisStatus('✓ 任务多端分解完毕。预仿真数学推演评估成功归档。');
                            reloadData();
                          }, 1000);
                        }}
                        className="px-3.5 py-1.5 bg-slate-800 hover:bg-slate-700 font-extrabold rounded text-xs text-white border border-slate-700 transition"
                      >
                        🧠 预仿真并分解原子任务
                      </button>
                      <button
                        onClick={() => {
                          setPlanAnalysisStatus(`正在组织董事会宪法合规检查并一键部署执行...`);
                          setTimeout(() => {
                            const res = autonomousPlanningService.executePlanningPipeline(selectedGoalId);
                            setPlanAnalysisStatus(`✓ 战略完全成功执行！受影响的系统状态变动已持久化落库：${res.actual_impact.substring(0, 100)}...`);
                            reloadData();
                          }, 1200);
                        }}
                        className="px-3.5 py-1.5 bg-emerald-500 hover:bg-emerald-600 font-extrabold rounded text-xs text-black transition"
                      >
                        🚀 批准宪法自检并一键部署执行
                      </button>
                    </div>
                  ...
                  {/* Tasks and Simulation Row */}
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    {/* Tasks Decomposed */}
                    <div className="space-y-3">
                      <h5 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                        <Bot className="w-4 h-4 text-[#07C2E3]" /> Multi-Agent 协作分解指令集 ({relatedTasks.length})
                      </h5>
                      <div className="space-y-2.5">
                        {relatedTasks.length === 0 ? (
                          <div className="py-12 border border-dashed border-slate-800 rounded-xl text-center text-slate-500 text-xs text-slate-600">
                            未生成子任务。请点击上方评估按钮启动多Agent角色分配。
                          </div>
                        ) : (
                          relatedTasks.map((task) => (
                            <div key={task.task_id} className="p-3 bg-slate-950 border border-slate-850 rounded-xl flex items-start justify-between gap-4">
                              <div className="space-y-1">
                                <div className="flex items-center gap-2">
                                  <span className="w-1.5 h-1.5 rounded-full bg-[#07C2E3]" />
                                  <span className="text-[10px] font-mono text-[#07C2E3] font-bold uppercase">
                                    Agent ID: {task.owner_agent}
                                  </span>
                                </div>
                                <p className="text-xs text-slate-300 leading-relaxed">{task.description}</p>
                              </div>
                              <span className={`text-[9px] px-2 py-0.5 rounded font-black uppercase ${
                                task.status === 'completed' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-yellow-500/10 text-yellow-400 border border-yellow-500/20'
                              }`}>
                                {task.status}
                              </span>
                            </div>
                    ...
                    <div className="space-y-3 bg-slate-950/50 border border-slate-850 p-4 rounded-xl">
                      <h5 className="text-xs font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5">
                        <Compass className="w-4 h-4 text-[#07C2E3] animate-spin-slow" /> Strategic Simulation Evaluation
                      </h5>
                      {relatedRun ? (
                        <div className="space-y-4 text-xs">
                          <div className="space-y-1.5 p-3 bg-slate-950 border border-slate-800 rounded-lg">
                            <span className="text-[10px] text-slate-500 font-bold uppercase block">方案整体提要:</span>
                            <p className="text-slate-300 leading-relaxed">{relatedRun.plan_summary}</p>
                          </div>

                          <div className="space-y-1.5 p-3 bg-slate-950 border border-slate-800 rounded-lg">
                            <span className="text-[10px] text-slate-500 font-bold uppercase block">预期宏观利益 (Pre-Run Forecast):</span>
                            <p className="text-[#07C2E3] font-mono leading-relaxed">{relatedRun.expected_impact}</p>
                          </div>

                          <div className="space-y-1.5 p-3 bg-slate-950 border border-slate-800 rounded-lg">
                            <span className="text-[10px] text-slate-500 font-bold uppercase block">实际落地核验 (Actual Result Log):</span>
                            <p className="text-slate-400 leading-relaxed">{relatedRun.actual_impact}</p>
                          </div>

                          <div className="flex items-center justify-between border-t border-slate-900 pt-3">
                            <span className="text-[10px] text-slate-500">模拟推演状态 (Run ID: {relatedRun.run_id})</span>
                            <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded font-mono ${
                              relatedRun.status === 'success' ? 'text-emerald-400' : 'text-yellow-400'
                            }`}>
                              {relatedRun.status}
                            </span>
                          </div>
