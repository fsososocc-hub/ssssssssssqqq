...
import { DNAGovernanceService } from '../../../services/DNAGovernanceService';
import { EvolutionService } from '../../../services/EvolutionService';
import { EnterpriseNervousSystemService } from '../../../services/EnterpriseNervousSystemService';
...
export function MemoryConsolidationLayer({ triggerSuccess }: LayerProps) {
  ...
  // Knowledge Validation Runtime parameters
  const [subTab, setSubTab] = useState<'memories' | 'validation' | 'dna' | 'evolution' | 'nervous' | 'governor'>('memories');
  const [selectedMemId, setSelectedMemId] = useState('');
  const [valTitle, setValTitle] = useState('');
  const [valType, setValType] = useState<'fact' | 'business' | 'strategy' | 'operation' | 'learning'>('business');
  const [valAgent, setValAgent] = useState('Strategic Validator Agent');

  // DNA Constitutional States
  const [dnaRuleName, setDnaRuleName] = useState('');
  const [dnaRuleDesc, setDnaRuleDesc] = useState('');
  const [dnaRuleType, setDnaRuleType] = useState<'business' | 'finance' | 'inventory' | 'risk' | 'permission' | 'compliance'>('business');
  const [dnaRulePriority, setDnaRulePriority] = useState<number>(3);

  // Nervous System interactive parameters
  const [evtType, setEvtType] = useState<'world_state' | 'tool' | 'agent' | 'memory' | 'knowledge' | 'dna' | 'evolution' | 'audit'>('world_state');
  const [evtSource, setEvtSource] = useState('World Thermometer API Sensor');
  const [evtRuntime, setEvtRuntime] = useState('WorldStateRuntime');
  const [evtPayload, setEvtPayload] = useState('{"country": "Germany", "temperature": -4.2, "alert": "Thermal drop anomaly"}');
  const [evtPriority, setEvtPriority] = useState<'low' | 'medium' | 'high' | 'critical'>('high');

  const [subName, setSubName] = useState('Marketing Optimizer Agent');
  const [subEvtType, setSubEvtType] = useState<'world_state' | 'tool' | 'agent' | 'memory' | 'knowledge' | 'dna' | 'evolution' | 'audit' | 'all'>('world_state');
  const [subPriority, setSubPriority] = useState<number>(2);

  const [directMsgSender, setDirectMsgSender] = useState('Pricing Engine Agent');
  const [directMsgReceiver, setDirectMsgReceiver] = useState('Inventory Master Agent');
  const [directMsgContent, setDirectMsgContent] = useState('Dynamic price modification approved by consensus.');

  // Simulation run selected target
  const [selectedCandidateId, setSelectedCandidateId] = useState('');

  // Governor Runtime local form states
  const [govPolName, setGovPolName] = useState('');
  const [govPolDesc, setGovPolDesc] = useState('');
  const [govPolType, setGovPolType] = useState<'financial_limit' | 'inventory_risk' | 'ad_spend' | 'compliance' | 'permission_escalation'>('financial_limit');
  const [govPolRisk, setGovPolRisk] = useState<'low' | 'medium' | 'high' | 'critical'>('high');
  const [govPolMode, setGovPolMode] = useState<'automatic' | 'manual_admin' | 'multi_signature' | 'emergency_bypass'>('manual_admin');

  // Governor proposal simulator fields
  const [simTaskId, setSimTaskId] = useState('task_proposal_99');
  const [simSource, setSimSource] = useState('Dynamic Pricing Agent');
  const [simActionType, setSimActionType] = useState<'price_change' | 'stock_purchase' | 'ad_campaign' | 'system_config' | 'compliance_check'>('price_change');
  const [simValue, setSimValue] = useState<number>(35); // representative parameter (e.g., discount % or cost budget)

  // Breaker triggers input
  const [breakerReasonInput, setBreakerReasonInput] = useState('Abnormal multi-agent discount feedback loop detected.');
  ...
  return (
    <div className="space-y-6 animate-fadeIn" id="memory_consolidation_layer">
      ...
      {/* Sub-tabs to choose between Memory Storage and Knowledge validation runtime */}
      <div className="flex flex-wrap border-b border-rose-50/10 gap-1 bg-slate-950 p-1 rounded-lg">
        <button
          onClick={() => setSubTab('memories')}
          className={`px-3 py-1.5 text-xs font-bold rounded-md font-sans transition flex items-center gap-1.5 ${
            subTab === 'memories' 
              ? 'bg-[#07C2E3] text-black shadow' 
              : 'text-slate-400 hover:text-white'
          }`}
        >
          <span>🧠 Memory Repository (内存库)</span>
          <span className="bg-slate-900 text-slate-300 text-[9px] px-1.5 py-0.2 rounded-full border border-slate-800">
            {rawMemories.length}
          </span>
        </button>
        <button
        ...
        >
          <span>🛡️ Knowledge Validation Hub (质检中枢)</span>
          <span className="bg-slate-900 text-[#07C2E3] text-[9px] px-1.5 py-0.2 rounded-full border border-slate-800 font-mono">
            {dbEngine.knowledge_records.getAll().length} Verified
          </span>
        </button>
        <button
        ...
        >
          <span>📜 DNA Constitutional Governance (企业宪法)</span>
          <span className="bg-slate-900 text-rose-400 text-[9px] px-1.5 py-0.2 rounded-full border border-slate-800 font-mono">
            {dbEngine.dna_violations.getAll().length} Violations
          </span>
        </button>
        <button
        ...
        >
          <span>🧬 Evolutionary Engine (进化中心)</span>
          <span className="bg-slate-900 text-green-400 text-[9px] px-1.5 py-0.2 rounded-full border border-slate-800 font-mono">
            {dbEngine.evolution_candidates.getAll().filter(c => c.status === 'evolved').length} Evolved
          </span>
        </button>
        <button
          onClick={() => setSubTab('nervous')}
          className={`px-3 py-1.5 text-xs font-bold rounded-md font-sans transition flex items-center gap-1.5 ${
            subTab === 'nervous' 
              ? 'bg-[#07C2E3] text-black shadow' 
              : 'text-slate-400 hover:text-white'
        ...
        >
          <span>🧠 Enterprise Nervous System (企业神经系统)</span>
          <span className="bg-slate-900 text-[#07C2E3] text-[9px] px-1.5 py-0.2 rounded-full border border-slate-800 font-mono">
            {dbEngine.nervous_events.getAll().length} Events
          </span>
        </button>
        <button
          onClick={() => setSubTab('governor')}
          className={`px-3 py-1.5 text-xs font-bold rounded-md font-sans transition flex items-center gap-1.5 ${
            subTab === 'governor' 
              ? 'bg-[#07C2E3] text-black shadow' 
              : 'text-slate-400 hover:text-white'
        ...
        >
          <span>⚖️ Enterprise Governor (最终裁决厅)</span>
          <span className="bg-slate-900 text-rose-400 text-[9px] px-1.5 py-0.2 rounded-full border border-slate-800 font-mono">
            {dbEngine.governor_policies.getAll().length} Policies
          </span>
        </button>
      </div>
      ...
      {subTab === 'memories' ? (
          ...
          <div className="bg-white border border-slate-205 p-5 rounded-xl border border-slate-200 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="md:col-span-3">
                <label className="block text-[10px] text-slate-505 uppercase font-bold font-mono mb-1">Memory Content Description</label>
                <textarea
                  value={customContent}
              ...
              <div>
                <label className="block text-[10px] text-slate-505 uppercase font-bold font-mono mb-1">Memory Type</label>
                <select
                  value={customType}
              ...
              <div>
                <label className="block text-[10px] text-slate-505 uppercase font-bold font-mono mb-1">Source Agent</label>
                <input
                  type="text"
                  value={customSource}
              ...
              <div>
                <label className="block text-[10px] text-slate-505 uppercase font-bold font-mono mb-1">Related Entity Identifier</label>
                <input
                  type="text"
                  value={customEntity}
              ...
              <div>
                <label className="block text-[10px] text-slate-505 uppercase font-bold font-mono mb-1">Importance Weight (1-10)</label>
                <input
                  type="number"
                  min={1}
                  max={10}
        ...
        /* Knowledge Validation Runtime (P0-006) Panel */
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-fadeIn text-slate-800">
          {/* Left Column (8 cols): Manual Validator Validation & Record Grid */}
          <div className="lg:col-span-8 space-y-6">
            {/* 1. Manual validation form */}
            <div className="bg-white border border-slate-200 p-5 rounded-xl shadow-sm space-y-4 animate-fadeIn">
              <div className="flex justify-between items-center border-b border-rose-50 pb-2">
                <h4 className="font-sans font-bold text-slate-900 flex items-center gap-1.5 text-sm">
                  <span className="text-cyan-500">🛡️</span> Promotion & Audit Consensus (长期知识提炼)
                </h4>
                <button
                  onClick={() => {
                    const sweepCount = KnowledgeService.getInstance().autoSweepValidation('merchant_paris_01', 'Validation AI Core');
                    triggerSuccess(`Auto-Sweep complete: ${sweepCount} unverified memories consolidated into standard enterprise knowledge.`);
                  }}
                  className="bg-slate-950 text-[#07C2E3] hover:text-white font-sans font-bold text-[10px] px-3 py-1.5 rounded transition font-mono border border-slate-800"
                >
                  ⚡ Run Automated Sweep Validator 
                </button>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                <div className="md:col-span-6">
                  <label className="block text-[10px] text-slate-500 uppercase font-bold font-mono mb-1">Select Candidate Memory</label>
                  {(() => {
                    const alreadyValidatedIds = new Set(dbEngine.knowledge_records.getAll().flatMap(k => k.source_memory_ids));
                    const unvalidated = rawMemories.filter(m => !alreadyValidatedIds.has(m.memory_id));
                    return (
                      <select
                        value={selectedMemId}
                        onChange={e => {
                          ...
                          setSelectedMemId(id);
                          const matchingMemory = unvalidated.find(m => m.memory_id === id);
                          if (matchingMemory) {
                            setValTitle(`CONSENSUS: ${matchingMemory.content.slice(0, 40)}...`);
                          }
                        }}
                        className="w-full text-xs p-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:border-[#07C2E3]"
                      >
                        <option value="">-- Choose an unverified memory item ({unvalidated.length} available) --</option>
                        {unvalidated.map(m => (
                          <option key={m.memory_id} value={m.memory_id}>
                            [{m.memory_type.toUpperCase()}] {m.content.slice(0, 75)}...
                          </option>
                        ))}
                      </select>
                    );
                ...
                <div className="md:col-span-6">
                  <label className="block text-[10px] text-slate-500 uppercase font-bold font-mono mb-1">Proposed Knowledge Title</label>
                  <input
                    type="text"
                    value={valTitle}
                    onChange={e => setValTitle(e.target.value)}
                    placeholder="e.g. French winter coat sales baseline"
                    className="w-full text-xs p-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:border-[#07C2E3]"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] text-slate-500 uppercase font-bold font-mono mb-1">Logical Knowledge Category</label>
                  <select
                    value={valType}
                  ...
                  >
                    ...
                    <option value="business">Business Metrics (商业经营经验)</option>
                    <option value="strategy">Strategy Decision (宏观决策逻辑)</option>
                    <option value="operation">Operational Directive (操作指令规范)</option>
                    <option value="learning">Heuristic Learning (多期对比学习)</option>
                  </select>
                </div>
                <div>
                  <label className="block text-[10px] text-slate-500 uppercase font-bold font-mono mb-1">Qualifying Validator Node</label>
                  <input
                    type="text"
                    value={valAgent}
              ...
              <div className="flex justify-end">
                <button
                  onClick={() => {
                    if (!selectedMemId) {
                      triggerSuccess("Error: Please select a candidate memory to promote.");
                      return;
                    }
                    if (!valTitle.trim()) {
                      triggerSuccess("Error: Proposed title for the new knowledge record cannot be blank.");
                      return;
                    }
                    const res = KnowledgeService.getInstance().validateMemoryToKnowledge({
                      merchant_id: 'merchant_paris_01',
                      memory_ids: [selectedMemId],
                      title: valTitle,
                      knowledge_type: valType,
                      validator_agent: valAgent
                    });
                    if (res.success) {
                      triggerSuccess(`Consensus approved: New core knowledge successfully validated under core registry index ${res.record?.knowledge_id}`);
                      setValTitle('');
                      setSelectedMemId('');
                    } else {
                      triggerSuccess(`Consensus rejected: Validation logs generated, confidence thresholds unmet.`);
                    }
                  }}
                  className="bg-[#07C2E3] hover:bg-[#06B2D0] text-black font-sans font-bold text-xs px-4 py-2 rounded-lg transition"
                >
                  Consolidate & Promote to Verified Knowledge Base
                </button>
              </div>
            </div>

            {/* 2. Knowledge records listing */}
            <div className="bg-white border text-neutral-800 border-slate-200 p-5 rounded-xl shadow-sm space-y-4">
              <h4 className="font-sans font-bold text-slate-900 flex items-center gap-2 text-sm">
                📁 Core Enterprise Knowledge Base (已确认高置信度核心知识)
              </h4>

              <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
                {dbEngine.knowledge_records.getAll().length === 0 ? (
                  <div className="py-12 text-center text-slate-400 text-xs">
                    No verified core knowledge records currently populated. Run auto-sweep above to harvest memories!
                  </div>
                ) : (
                  dbEngine.knowledge_records.getAll().map(record => {
                    let badgeK = "bg-sky-50 text-sky-700 border-sky-100";
                    if (record.knowledge_type === 'strategy') badgeK = "bg-purple-50 text-purple-700 border-purple-100";
                    if (record.knowledge_type === 'operation') badgeK = "bg-amber-50 text-amber-700 border-amber-100";
                    if (record.knowledge_type === 'learning') badgeK = "bg-rose-50 text-rose-700 border-rose-100";

                    return (
                      <div key={record.knowledge_id} className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-3 relative hover:shadow-sm transition text-xs">
                        <div className="flex justify-between items-start gap-4 flex-wrap">
                          <div className="space-y-1">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span className="font-mono text-slate-400 font-bold">[{record.knowledge_id}]</span>
                              <h5 className="font-sans font-bold text-slate-900 text-xs">{record.title}</h5>
                              <span className={`px-1.5 py-0.5 text-[8px] uppercase font-bold rounded border ${badgeK}`}>
                                {record.knowledge_type}
                              </span>
                            </div>
                            <p className="text-slate-600 text-xs mt-1 leading-relaxed">{record.content}</p>
                          </div>

                          <div className="flex flex-col items-end gap-1 shrink-0">
                            {record.status === 'approved' && (
                              <span className="bg-emerald-50 text-emerald-700 text-[9px] font-bold px-1.5 py-0.5 rounded border border-emerald-100 font-sans">
                                ● APPROVED
                              </span>
                            )}
                            {record.status === 'conflicted' && (
                              <span className="bg-rose-50 text-rose-700 text-[9px] font-bold px-1.5 py-0.5 rounded border border-rose-100 animate-pulse font-sans font-mono">
                                ● CONFLICTED
                              </span>
                            )}
                            {record.status === 'deprecated' && (
                              <span className="bg-slate-100 text-slate-500 text-[9px] font-bold px-1.5 py-0.5 rounded border border-slate-200 line-through font-sans">
                                ● DEPRECATED
                              </span>
                            )}
                          </div>
                        ...
                        {/* Stats block & gauge bar */}
                        <div className="flex flex-wrap items-center justify-between text-[10px] text-slate-400 bg-white border border-slate-200 p-2 rounded-lg gap-4">
                          <div className="flex items-center gap-3 flex-wrap">
                            <span className="flex items-center gap-1">
                              Confidence: <strong className="text-slate-700 font-bold">{(record.confidence * 100).toFixed(0)}%</strong>
                            </span>
                            <span className="text-slate-200">|</span>
                            <span className="flex items-center gap-1">
                              Verification Score: <strong className="text-[#07C2E3] font-mono font-bold">{record.validation_score}/100</strong>
                            </span>
                            <span className="text-slate-200">|</span>
                            <span className="truncate">Source Memories: {record.source_memory_ids.join(', ')}</span>
                          </div>

                          <div className="flex items-center gap-1.5">
                            <button
                              onClick={() => {
                                KnowledgeService.getInstance().recalibrateConfidence(record.knowledge_id);
                                triggerSuccess(`Recalibration complete. Adjusted confidence weighting across dynamic time decays and logs.`);
                              }}
                              className="bg-white hover:bg-slate-100 border border-slate-200 text-slate-700 px-2 py-1 rounded text-[9px] font-medium transition"
                            >
                              ⏳ Recalibrate
                            </button>
                            <button
                              onClick={() => {
                                dbEngine.knowledge_records.update(record.knowledge_id, { status: 'deprecated' });
                                triggerSuccess(`Record marked as deprecated successfully.`);
                              }}
                              className="bg-white hover:bg-rose-50 border border-slate-200 text-rose-700 px-2 py-1 rounded text-[9px] font-medium transition"
                            >
                              🚫 Deprecate
                            </button>
                            <button
                              onClick={() => {
                                dbEngine.knowledge_records.delete(record.knowledge_id);
                                triggerSuccess(`Knowledge record purged from database successfully.`);
                              }}
          ...
          {/* Right Column (4 cols): Conflict sweep control and quality validation log panel */}
          <div className="lg:col-span-4 space-y-6">
            {/* 1. Conflict Sweeper control */}
            <div className="bg-[#0b1527] border border-slate-850 p-5 rounded-xl text-white space-y-3 shadow-xl">
              <span className="text-[#07C2E3] text-[9px] font-mono block uppercase tracking-wide font-extrabold pb-0.5">Overlap Conflict Detector</span>
              <h5 className="font-sans font-bold text-sm text-white">Semantic Contradiction Auditer</h5>
              <p className="text-[11px] text-slate-400 leading-relaxed">
                主动监控知识库中重叠、过时、或存在逻辑矛盾的项目。冲突项目将被主动屏蔽防止 Agent 采用。
              </p>
              
              <button
                onClick={() => {
                  const conflictResult = KnowledgeService.getInstance().detectConflicts('merchant_paris_01');
                  triggerSuccess(`Sweeper completed. Conflicts flagged: ${conflictResult.conflictsFound}. Blocked overlapping parameters.`);
                }}
                className="w-full bg-[#07C2E3] hover:bg-[#06B2D0] text-black font-sans font-bold text-xs py-2 rounded-lg transition"
              >
                Trigger Semantic Conflict Sweeper
              </button>
            </div>

            {/* 2. Validation Audit logs */}
            <div className="bg-slate-950 border border-slate-850 p-5 rounded-xl text-white shadow-xl space-y-4 font-sans">
              <div>
                <span className="text-[10px] text-[#07C2E3] font-mono font-bold uppercase tracking-wider block">Consensus Auditor</span>
                <h4 className="font-sans font-bold text-sm text-white mt-0.5">Knowledge Audit Trails</h4>
              </div>

              <div className="space-y-2 max-h-[420px] overflow-y-auto pr-1">
                {dbEngine.knowledge_validation_logs.getAll().length === 0 ? (
                  <div className="text-center py-12 text-slate-600 text-[11px]">
                    No validation log records generated yet. Prompt unverified memories above to validate!
                  </div>
                ) : (
                  dbEngine.knowledge_validation_logs.getAll().sort((a,b)=>b.created_at.localeCompare(a.created_at)).map(log => {
                    const isSuccess = log.result === 'success';
                    return (
                      <div key={log.log_id} className="p-2.5 rounded bg-slate-900 border border-slate-850 space-y-1.5 text-[11px] text-left">
                        <div className="flex justify-between items-center flex-wrap gap-1">
                          <span className={`text-[8.5px] font-extrabold uppercase px-1 rounded ${
                            isSuccess ? 'bg-emerald-950/45 text-emerald-400' : 'bg-amber-950/45 text-amber-500'
                          }`}>
                        ...
                        <div className="text-slate-300">
                          Validator node <strong className="text-white font-mono">{log.validator}</strong> executed dimension: <strong className="text-cyan-400 font-mono">{log.validation_type}</strong>
                        </div>

                        <p className="text-slate-400 leading-normal text-[10px] bg-slate-950/50 p-1.5 rounded border border-slate-850 text-slate-400">
                          {log.reason}
                        </p>

                        <div className="text-[9px] text-slate-500 flex gap-2 justify-end">
                          <span>Confidence curve: {log.confidence_before.toFixed(2)} → {log.confidence_after.toFixed(2)}</span>
                        </div>
                      </div>
                    );
                  })
        ...
        /* DNA Governance (P0-007) Panel */
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-fadeIn text-slate-800">
          {/* Left Side (8 cols): Interactive Rule Creation/Controls + Constraints Visual Repository */}
          <div className="lg:col-span-8 space-y-6">
            {/* 1. Constitution Rule Establisher Form */}
            <div className="bg-white border border-slate-200 p-5 rounded-xl shadow-sm space-y-4">
              <h4 className="font-sans font-bold text-slate-900 flex items-center gap-2 text-sm">
                <span>📜 Establish New Constitutional DNA Constraint</span>
              </h4>

              <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                <div className="md:col-span-6">
                  <label className="block text-[10px] text-slate-500 uppercase font-bold font-mono mb-1">Rule Identifier / Name</label>
                  <input
                    type="text"
                    value={dnaRuleName}
                    onChange={e => setDnaRuleName(e.target.value)}
                    placeholder="e.g. Holiday Season Promotion Floor Locked"
                    className="w-full text-xs p-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:border-[#07C2E3]"
                  />
                </div>

                <div className="md:col-span-6">
                  <label className="block text-[10px] text-slate-500 uppercase font-bold font-mono mb-1">Rule Domain Classification</label>
                  <select
                    value={dnaRuleType}
                    onChange={e => setDnaRuleType(e.target.value as any)}
                    className="w-full text-xs p-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:border-[#07C2E3]"
                  >
                    <option value="business">Business Rule (业务规则)</option>
                    <option value="finance">Financial Limits (财务规则)</option>
                    <option value="inventory">Inventory Safeguards (库存安全原则)</option>
                    <option value="risk">Risk Concentration (风险承受边界)</option>
                    <option value="permission">A AIAgent Permissions (权限审查)</option>
                    <option value="compliance">Regulatory Compliance (合规合律律)</option>
                  </select>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-4">
                <div className="md:col-span-8">
                  <label className="block text-[10px] text-slate-500 uppercase font-bold font-mono mb-1">Irremovable Rule Mandate / Description</label>
                  <textarea
                    rows={2}
                    value={dnaRuleDesc}
                    onChange={e => setDnaRuleDesc(e.target.value)}
                    placeholder="Describe exact operational restriction, values, and fallback thresholds checked."
                    className="w-full text-xs p-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:border-[#07C2E3]"
                  />
                </div>

                <div className="md:col-span-4">
                  <label className="block text-[10px] text-slate-500 uppercase font-bold font-mono mb-1">Priority (1 = Fatal Block)</label>
                  <select
                    value={dnaRulePriority}
                    onChange={e => setDnaRulePriority(Number(e.target.value))}
                    className="w-full text-xs p-2 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:border-[#07C2E3]"
                  >
                    <option value="1">1 - Fatal Block Control</option>
                    <option value="2">2 - High Warning Escalation</option>
                    <option value="3">3 - Standard Sandbox Review</option>
                    <option value="4">4 - Informative Notice Log</option>
                  </select>
                </div>
              </div>
              ...
              <div className="flex justify-end pt-1">
                <button
                  onClick={() => {
                    if (!dnaRuleName.trim() || !dnaRuleDesc.trim()) {
                      triggerSuccess("Error: Name and description are required to encode a constitution rule.");
                      return;
                    }
                    dbEngine.dna_rules.create({
                      rule_name: dnaRuleName,
                      rule_type: dnaRuleType,
                      description: dnaRuleDesc,
                      priority: dnaRulePriority,
                      status: 'active'
                    });
                    setDnaRuleName('');
                    setDnaRuleDesc('');
                    triggerSuccess("Constitution Rule successfully encoded into hard iron database. Active on all Agent layers.");
                  }}
                  className="bg-slate-950 text-[#07C2E3] hover:text-white font-sans font-bold text-xs px-4 py-2 rounded-lg border border-slate-800 transition"
                >
                  🧬 Establish DNA Constitution Rule
                </button>
              </div>
            </div>

            {/* 2. Active Constitution Rule Cards */}
            <div className="bg-white border border-slate-200 p-5 rounded-xl shadow-sm space-y-4">
              <h4 className="font-sans font-bold text-slate-900 flex items-center justify-between text-sm">
                <span>📁 Core Constitutional Clauses Repository ({dbEngine.dna_rules.getAll().length} clauses)</span>
                <span className="text-[10px] font-mono text-slate-400 font-bold uppercase">Unbreachable Rules</span>
              </h4>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {dbEngine.dna_rules.getAll().map(rule => {
                  let badgeType = "bg-slate-50 text-slate-700 border-slate-200";
                  if (rule.rule_type === 'finance') badgeType = "bg-rose-50 text-rose-700 border-rose-100";
                  if (rule.rule_type === 'inventory') badgeType = "bg-amber-50 text-amber-700 border-amber-100";
                  if (rule.rule_type === 'compliance') badgeType = "bg-sky-50 text-sky-700 border-sky-100";
                  if (rule.rule_type === 'risk') badgeType = "bg-purple-50 text-purple-700 border-purple-100";

                  return (
                    <div key={rule.rule_id} className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-2 hover:shadow-sm transition flex flex-col justify-between">
                      <div className="space-y-1.5 text-xs">
                        <div className="flex justify-between items-center flex-wrap gap-1">
                          <span className="font-mono text-slate-400 text-[10px] font-bold">[{rule.rule_id}]</span>
                          <span className={`px-1.5 py-0.5 text-[8.5px] font-sans font-bold rounded uppercase border ${badgeType}`}>
                            {rule.rule_type}
                          </span>
                        </div>
                        <h5 className="font-sans font-bold text-slate-900 text-xs">{rule.rule_name}</h5>
                        <p className="text-slate-500 text-[11px] leading-relaxed">{rule.description}</p>
                      </div>

                      <div className="flex justify-between items-center text-[10px] text-slate-400 bg-white border border-slate-150 p-1.5 rounded-lg mt-3">
                        <span className="font-mono">Priority: <strong>{rule.priority}</strong></span>
                        <div className="flex gap-1.5">
                          <button
                            onClick={() => {
                              dbEngine.dna_rules.update(rule.rule_id, {
                                status: rule.status === 'active' ? 'inactive' : 'active'
                              });
                              triggerSuccess(`Clause status of ${rule.rule_id} changed successfully.`);
                            }}
                            className="text-slate-600 hover:text-black font-semibold text-[9px]"
                          >
                          ...
                          <button
                            onClick={() => {
                              dbEngine.dna_rules.delete(rule.rule_id);
                              triggerSuccess(`Clause ${rule.rule_id} purged from constitution.`);
                            }}
                            className="text-red-500 hover:text-red-700 font-semibold text-[9px]"
                          >
                            Purge
                          </button>
                        </div>
          ...
          {/* Right Column (4 cols): Interactive Scenarios Simulator & Real Violation logs */}
          <div className="lg:col-span-4 space-y-6">
            {/* 1. Interactive Sandbox Violator Simulator */}
            <div className="bg-[#0b1527] border border-slate-850 p-5 rounded-xl text-white space-y-3 shadow-xl flex flex-col justify-between">
              <div>
                <span className="text-[#07C2E3] text-[9px] font-mono block uppercase tracking-wide font-extrabold pb-0.5">Scenario Ground Test Engine</span>
                <h5 className="font-sans font-bold text-sm text-white">Interactive DNA Guard Simulator</h5>
                <p className="text-[11px] text-slate-400 leading-relaxed">
                  Manually trigger real simulated actions to trace the unbreachable enforcement checks (Allow / Block / Escalate / Review) in real time.
                </p>
              </div>
              ...
              <div className="space-y-2 mt-2">
                <button
                  onClick={() => {
                    const result = DNAGovernanceService.getInstance().evaluateGovernanceSafety({
                      agent: 'Pricing AI Node',
                      tool: 'batch_modify_discount',
                      ruleType: 'finance',
                      inputData: { discount: 0.45, productId: 'prod_merino_01' }
                    });
                    if (result.action === 'block') {
                      triggerSuccess(`❌ PROACTIVE BLOCK: Pricing agent attempted 45% discount on Merino items. Constitution Rule dna_rule_01 triggered!`);
                    }
                  }}
                  className="w-full bg-[#182845] hover:bg-[#1f3458] text-slate-200 font-sans font-bold text-[10px] py-2 rounded text-left px-3 border border-slate-800 flex justify-between items-center transition"
                >
                  <span>1. Trigger 45% Pricing Discount Limit</span>
                  <span className="text-red-400 font-mono text-[9px] font-bold">BLOCK</span>
                </button>

                <button
                  onClick={() => {
                    const result = DNAGovernanceService.getInstance().evaluateGovernanceSafety({
                      agent: 'Fulfill Agent Master',
                      tool: 'issue_loyalty_giveaway',
                      ruleType: 'inventory',
                      inputData: { remainingQty: 3, allocationId: 'alloc_0029' }
                    });
                    if (result.action === 'block') {
                      triggerSuccess(`❌ PROACTIVE BLOCK: Safety buffer violated. Stock reduced to 3. Constitution Rule dna_rule_02 enforced!`);
                    }
                  }}
                  className="w-full bg-[#182845] hover:bg-[#1f3458] text-slate-200 font-sans font-bold text-[10px] py-2 rounded text-left px-3 border border-slate-800 flex justify-between items-center transition"
                >
                  <span>2. Deplete Stock Below Safety (Qty: 3)</span>
                  <span className="text-red-400 font-mono text-[9px] font-bold">BLOCK</span>
                </button>
                ...
                <button
                  onClick={() => {
                    const result = DNAGovernanceService.getInstance().evaluateGovernanceSafety({
                      agent: 'Promo Generator Node',
                      tool: 'generate_marketing_sms',
                      ruleType: 'business',
                      inputData: 'Click here now to win cheap replica shoes and limited time only discounts!'
                    });
                    if (result.action === 'review') {
                      triggerSuccess(`⚠️ ESCALATE REVIEW: Clickbait tone signature captured. Escalated for Editorial Review.`);
                    }
                  }}
                  className="w-full bg-[#182845] hover:bg-[#1f3458] text-slate-200 font-sans font-bold text-[10px] py-2 rounded text-left px-3 border border-slate-800 flex justify-between items-center transition"
                >
                  <span>3. Send Marketing Spam Tone SMS</span>
                  <span className="text-amber-400 font-mono text-[9px] font-bold">REVIEW</span>
                </button>

                <button
                  onClick={() => {
                    const result = DNAGovernanceService.getInstance().evaluateGovernanceSafety({
                      agent: 'Global SCM Planner',
                      tool: 're_route_transit_lines',
                      ruleType: 'risk',
                      inputData: { allocationRatio: 0.85, supplierRegion: 'milano_dry_inter' }
                    });
                    if (result.action === 'escalate') {
                      triggerSuccess(`⚠️ ENFORCEMENT WARN: Geodiversity concentration exceeded risk ceiling (85%). Escalated to Admin Board.`);
                    }
                  }}
                  className="w-full bg-[#182845] hover:bg-[#1f3458] text-slate-200 font-sans font-bold text-[10px] py-2 rounded text-left px-3 border border-slate-800 flex justify-between items-center transition"
                >
                  <span>4. Concentrate Sourcing (85% Limit)</span>
                  <span className="text-purple-400 font-mono text-[9px] font-bold">ESCALATE</span>
                </button>

                <button
                  onClick={() => {
                    const result = DNAGovernanceService.getInstance().evaluateGovernanceSafety({
                      agent: 'CRM Analytics Node',
                      tool: 'profile_demographic_mining',
                      ruleType: 'compliance',
                      inputData: 'Customer email is guest_paris@luxury.com'
                    });
                    if (result.action === 'block') {
                      triggerSuccess(`❌ PROACTIVE GDPR BLOCK: Unencrypted client mail signature caught in analytics pipeline. Blocked.`);
                    }
                  }}
                  className="w-full bg-[#182845] hover:bg-[#1f3458] text-slate-200 font-sans font-bold text-[10px] py-2 rounded text-left px-3 border border-slate-800 flex justify-between items-center transition"
                >
                  <span>5. Log Customer Email Raw leak</span>
                  <span className="text-red-400 font-mono text-[9px] font-bold">BLOCK</span>
                </button>
              </div>
            </div>

            {/* 2. Real-time Enforcement Audit Logs */}
            <div className="bg-slate-950 border border-slate-850 p-5 rounded-xl text-white shadow-xl space-y-4 font-sans">
              <div className="flex justify-between items-center">
                <div>
                  <span className="text-[10px] text-rose-400 font-mono font-bold uppercase tracking-wider block">Compliance Audit</span>
                  <h4 className="font-sans font-bold text-sm text-white mt-0.5">DNA Violation Audit Logs</h4>
                </div>
                {dbEngine.dna_violations.getAll().length > 0 && (
                  <button
                    onClick={() => {
                      dbEngine.dna_violations.purge();
                      triggerSuccess('Database violation audit trails flushed.');
                    }}
                    className="text-[9px] text-[#07C2E3] font-mono hover:underline"
                  >
                    Clear Logs
                  </button>
                )}
              ...
              <div className="space-y-2 max-h-[380px] overflow-y-auto pr-1">
                {dbEngine.dna_violations.getAll().length === 0 ? (
                  <div className="text-center py-12 text-slate-600 text-[11px]">
                    No constitutional violations logged yet. Establish rules or test simulations above!
                  </div>
                ) : (
                  dbEngine.dna_violations.getAll().sort((a,b)=>b.created_at.localeCompare(a.created_at)).map(viol => {
                    let sevColor = "bg-red-950/45 text-red-400 border border-red-900";
                    if (viol.severity === 'review') sevColor = "bg-amber-950/45 text-amber-400 border border-amber-900";
                    if (viol.severity === 'escalate') sevColor = "bg-purple-950/45 text-purple-400 border border-purple-900";
                    if (viol.severity === 'allow') sevColor = "bg-emerald-950/45 text-emerald-400 border border-emerald-900";

                    return (
                      <div key={viol.violation_id} className="p-2.5 rounded bg-slate-900 border border-slate-850 space-y-1.5 text-[11px] text-left">
                        <div className="flex justify-between items-center flex-wrap gap-1">
                          <span className={`text-[8.5px] font-extrabold uppercase px-1 rounded ${sevColor}`}>
                            {viol.severity.toUpperCase()}
                          </span>
                          <span className="font-mono text-[8px] text-slate-500">
                            {new Date(viol.created_at).toLocaleTimeString()}
                          </span>
                        </div>

                        <div className="text-slate-300">
                          Agent <strong className="text-white font-mono">{viol.agent}</strong> calling <strong className="text-[#07C2E3] font-mono">{viol.tool}</strong> triggered rule <strong className="text-pink-400 font-mono">{viol.rule_id}</strong>
                        </div>

                        <p className="text-slate-400 leading-normal text-[10px] bg-slate-950/50 p-2 rounded border border-slate-850">
                          {viol.reason}
                        </p>

                        <div className="text-[9px] text-slate-500 flex justify-between items-center">
                          <code className="bg-slate-950 text-slate-400 text-[8px] px-1 py-0.2 rounded font-mono truncate max-w-[70%]">{viol.input}</code>
                          {viol.severity !== 'allow' && (
                            <button
                              onClick={() => {
                                const ok = DNAGovernanceService.getInstance().handleManualExempt(viol.violation_id, 'SuperAdmin_Override');
                                if (ok) triggerSuccess(`Manual override checklist generated. Allowed operation bypass.`);
                              }}
                              className="text-[#07C2E3] hover:underline font-mono text-[8.5px]"
                            >
                              Exempt Override
                            </button>
                          )}
        ...
        /* Evolution Dynamic Panel (P0-008) */
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-fadeIn text-slate-800">
          {/* Left Side (8 cols): Evloution Candidates & Strategy Pattern Discovery Canvas */}
          <div className="lg:col-span-8 space-y-6">
            <div className="bg-white border border-slate-200 p-5 rounded-xl shadow-sm space-y-4">
              <div className="flex justify-between items-center border-b border-slate-100 pb-2 flex-wrap gap-2">
                <div className="space-y-0.5">
                  <h4 className="font-sans font-bold text-slate-900 flex items-center gap-1.5 text-sm">
                    <span className="text-green-500">🧬</span> Systemic Evolution Candidates & Proposals
                  </h4>
                  <p className="text-[10px] text-slate-400">Heuristic discovery triggers from memories, historical transitions, and raw business events.</p>
                </div>

                <div className="flex gap-2">
                  <button
                    onClick={() => {
                      const spawned = EvolutionService.getInstance().runPatternDiscovery();
                      triggerSuccess(`Evolution Scan complete: ${spawned} new potential strategy pattern discovered in memory repository!`);
                    }}
                    className="bg-slate-950 hover:bg-slate-900 text-[#07C2E3] font-sans font-bold text-[10px] px-3 py-1.5 rounded transition border border-slate-800"
                  >
              ...
              {/* Candidates Feed */}
              <div className="space-y-3 max-h-[500px] overflow-y-auto pr-1">
                {dbEngine.evolution_candidates.getAll().length === 0 ? (
                  <div className="py-12 text-center text-slate-400 text-xs">
                    No active evolution candidates populated. Click Pattern Discovery to harvest.
                  </div>
                ) : (
                  dbEngine.evolution_candidates.getAll().map(cand => {
                    const runs = dbEngine.evolution_runs.getAll().filter(r => r.candidate_id === cand.candidate_id);
                    const activeRun = runs.find(r => r.status === 'active' || r.status === 'simulating');

                    let badgeColor = "bg-slate-50 text-slate-700 border-slate-250";
                    if (cand.status === 'simulated') badgeColor = "bg-amber-50 text-amber-700 border-amber-100";
                    if (cand.status === 'evolved') badgeColor = "bg-green-50 text-green-700 border-green-100";
                    if (cand.status === 'rolled_back') badgeColor = "bg-rose-50 text-rose-700 border-rose-100";

                    return (
                      <div key={cand.candidate_id} className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-3 relative hover:shadow-sm transition text-xs">
                        <div className="flex justify-between items-start gap-4 flex-wrap">
                          <div className="space-y-1">
                            <div className="flex items-center gap-1.5 flex-wrap">
                              <span className="font-mono text-slate-400 font-bold">[{cand.candidate_id}]</span>
                              <span className="px-1.5 py-0.5 text-[8.5px] font-sans font-bold uppercase rounded border bg-white text-slate-700 border-slate-200">
                                Source: {cand.source}
                              </span>
                              <span className={`px-1.5 py-0.5 text-[8.5px] uppercase font-bold rounded border ${badgeColor}`}>
                                {cand.status}
                              </span>
                            </div>
                            <p className="text-slate-600 text-xs mt-1 leading-relaxed font-sans">{cand.description}</p>
                          </div>

                          <div className="shrink-0 flex flex-col items-end gap-1">
                            <div className="text-[10px] text-slate-400">Confidence Match</div>
                            <div className="font-mono text-slate-800 font-bold text-sm">{(cand.confidence * 100).toFixed(0)}%</div>
                          </div>
                        </div>

                        {/* Visual statistics indicator & Actions bar */}
                        <div className="flex flex-wrap items-center justify-between text-[10px] text-slate-400 bg-white border border-slate-200 p-2.5 rounded-lg gap-4">
                          <div className="flex items-center gap-3">
                            <span>Predicted ROI Impact Modifier: <strong className="text-slate-700">+{cand.impact_score}%</strong></span>
                          </div>
                          ...
                          <div className="flex items-center gap-1.5">
                            {cand.status === 'pending' && (
                              <button
                                onClick={() => {
                                  const run = EvolutionService.getInstance().simulateCandidateEvolution(cand.candidate_id);
                                  if (run) triggerSuccess(`Interactive Simulation Booted. Impact matrix logged. Run index: ${run.run_id}`);
                                }}
                                className="bg-amber-500 hover:bg-amber-600 text-black font-sans font-bold px-2.5 py-1 rounded text-[10px] transition"
                              >
                                🧪 Evaluate Simulation
                              </button>
                            )}

                            {cand.status === 'simulated' && activeRun && (
                              <button
                                onClick={() => {
                                  EvolutionService.getInstance().executeEvolutionarySuccess(cand.candidate_id, activeRun.run_id);
                                  triggerSuccess('Evolve safe: Strategy compiled, verified, and mapped to Live Business Knowledge Registry.');
                                }}
                                className="bg-[#07C2E3] hover:bg-[#06B2D0] text-black font-sans font-bold px-2.5 py-1 rounded text-[10px] transition"
                              >
                                🧬 Authorize Evolve Production
                              </button>
                            )}

                            {cand.status === 'evolved' && activeRun && (
                              <button
                                onClick={() => {
                                  EvolutionService.getInstance().rollbackEvolutionRun(cand.candidate_id, activeRun.run_id);
                                  triggerSuccess('Emergency Rollback: Strategy policy deactivated, core guidelines reverted safely.');
                                }}
                                className="bg-red-500 hover:bg-red-600 text-white font-sans font-bold px-2.5 py-1 rounded text-[10px] transition"
                              >
                                🚫 Trigger Active Rollback
                              </button>
                            )}

                            <button
                              onClick={() => {
                                dbEngine.evolution_candidates.delete(cand.candidate_id);
                                triggerSuccess('Candidate proposal discarded.');
                              }}
                              className="text-slate-400 hover:text-red-500 p-0.5"
                            >
                              🗑️
          ...
          {/* Right Side (4 cols): Sandboxed runs monitoring & simulation score charts */}
          <div className="lg:col-span-4 space-y-6 text-white font-sans">
            {/* 1. Sandboxed Simulation Monitor board */}
            <div className="bg-[#0b1527] border border-slate-850 p-5 rounded-xl text-white space-y-3 shadow-xl">
              <div>
                <span className="text-[#07C2E3] text-[9px] font-mono block uppercase tracking-wide font-extrabold pb-0.5">Sandbox Sandbox Evaluation</span>
                <h5 className="font-sans font-bold text-sm text-white">Simulation Core Monitor</h5>
                <p className="text-[11px] text-slate-400 leading-relaxed">
                  Every evolved rule must undergo safe sandboxing internally. Relies on Monte Carlo modeling to estimate ROI stability.
                </p>
              </div>

              {/* Simulation metrics list */}
              <div className="space-y-3 max-h-[400px] overflow-y-auto mt-2 pr-1">
                {dbEngine.evolution_runs.getAll().length === 0 ? (
                  <div className="text-slate-500 text-[10px] py-4 text-center">
                    No active sandbox simulation trials executing.
                  </div>
                ) : (
                  dbEngine.evolution_runs.getAll().map(run => {
                    const cand = dbEngine.evolution_candidates.getById(run.candidate_id);
                    let resultBadge = "bg-green-950/45 text-green-400";
                    if (run.result === 'rolled_back') resultBadge = "bg-rose-950/45 text-rose-400";

                    return (
                      <div key={run.run_id} className="p-3 bg-slate-900 border border-slate-850 rounded-lg text-[11px] space-y-1.5">
                        <div className="flex justify-between items-center">
                          <span className="font-mono text-[9px] text-[#07C2E3] font-bold">[{run.run_id}]</span>
                          <span className={`px-1 py-0.2 rounded text-[8px] font-bold uppercase ${resultBadge}`}>
                            {run.result.toUpperCase()}
                          </span>
                        </div>
                        <div className="text-slate-300">
                          Target: <strong className="text-white font-mono">{run.candidate_id}</strong>
                        </div>
                        <p className="text-[10px] text-slate-400 italic">"{cand ? cand.description.slice(0, 50) : 'Ecosystem metrics estimation'}..."</p>
                        
                        {/* Meter progress compare */}
                        <div className="space-y-1 pt-1">
                          <div className="flex justify-between text-[8.5px] text-slate-500 font-mono">
                            <span>Pre-Simulation Baseline: {run.impact_before}%</span>
                            <span>Target Projection: {run.impact_after}%</span>
                          </div>
                          <div className="h-1.5 w-full bg-slate-950 rounded border border-slate-850 overflow-hidden relative flex">
                            <div className="bg-slate-700 h-full" style={{ width: `${run.impact_before}%` }}></div>
                            <div className="bg-[#07C2E3] h-full" style={{ width: `${run.impact_after - run.impact_before}%` }}></div>
                          </div>
                          <div className="text-right text-[8px] text-[#07C2E3]">Confidence Level: {cand ? (cand.confidence * 100).toFixed(0) : '90'}%</div>
                        </div>
                      </div>
                    );
                  })
            ...
            {/* 2. Evolution Audit trail */}
            <div className="bg-slate-950 border border-slate-850 p-5 rounded-xl text-white shadow-xl space-y-4 font-sans">
              <div>
                <span className="text-[10px] text-[#07C2E3] font-mono font-bold uppercase tracking-wider block">Evolutionary Audit Trace</span>
                <h4 className="font-sans font-bold text-sm text-white mt-0.5">Feedback Loop Logs</h4>
                <p className="text-[10px] text-slate-500 leading-relaxed mt-1">Ecosystem mutations, approved pathways, and compensation parameters.</p>
              </div>
              ...
              <div className="space-y-1.5 text-[10px] text-slate-400 font-mono">
                <div className="p-2 rounded bg-slate-900 border border-slate-850">
                  <span className="text-green-400 font-bold block">● [TRANSIT EXPANSION] SUCCESS</span>
                  <span>Evolved cold buffer baseline to 18 days. Adjusted pricing elasticities by +5%. Verified in sandbox.</span>
                </div>
                <div className="p-2 rounded bg-slate-900 border border-slate-850">
                  <span className="text-slate-400 font-bold block">● [KNOWLEDGE INTEGRITY FEEDBACK] COMPLETE</span>
                  <span>System modified physical catalog baseline of WMS models correctly.</span>
                </div>
              </div>
            </div>
          </div>
        ...
        /* P0-009 Enterprise Nervous System Real-time Console */
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-fadeIn text-slate-100">
          
          {/* LEFT 8 COLUMNS: Live Event Bus, Broadcasting & Preset Cascade Playground */}
          <div className="lg:col-span-8 space-y-6">
            
            {/* 1. Interactive Event Publisher */}
            <div className="bg-slate-950 border border-slate-850 p-6 rounded-xl shadow-xl space-y-4">
              <div className="flex justify-between items-start flex-wrap gap-2">
                <div>
                  <span className="text-[10px] text-[#07C2E3] font-mono font-bold uppercase tracking-widest block">Event Bus Core</span>
                  <h3 className="font-sans font-extrabold text-[#07C2E3] text-lg">Real-time Event Broadcast Station</h3>
                  <p className="text-[10.5px] text-slate-400 mt-1">Publish systemic state changes or hardware events and watch the cascade engine dispatch workflows to active subscribers.</p>
                </div>
                <div className="flex gap-2">
                  <button 
                    onClick={() => {
                      setEvtType('world_state');
                      setEvtSource('Thermostat-DE-WMS');
                      setEvtRuntime('WorldStateRuntime');
                      setEvtPayload('{"country": "Germany", "temperature": -4.2, "alert": "Thermal drop anomaly"}');
                      setEvtPriority('high');
                    }}
                    className="px-2.5 py-1 rounded bg-slate-900 border border-slate-800 text-[9.5px] hover:bg-slate-800 text-amber-400 font-mono transition"
                  >
                    ❄️ Cold Preset
                  </button>
                  <button 
                    onClick={() => {
                      setEvtType('tool');
                      setEvtSource('Inventory System Cron');
                      setEvtRuntime('ToolRuntime');
                      setEvtPayload('{"job_id": "cron_stock_sync_09", "affected_skus": ["SKU-COAT-WOOL-M"], "new_stock": 4}');
                      setEvtPriority('critical');
                    }}
                    className="px-2.5 py-1 rounded bg-slate-900 border border-slate-800 text-[9.5px] hover:bg-slate-800 text-rose-400 font-mono transition"
                  >
                    📦 Low Stock Preset
                  </button>
                  <button 
                    onClick={() => {
                      setEvtType('dna');
                      setEvtSource('DNA Governance Engine');
                      setEvtRuntime('DNAGovernanceRuntime');
                      setEvtPayload('{"violation_id": "v_seed_1", "rule_id": "dna_rule_01", "action": "block"}');
                      setEvtPriority('critical');
                    }}
                    className="px-2.5 py-1 rounded bg-slate-900 border border-slate-800 text-[9.5px] hover:bg-slate-800 text-red-500 font-mono transition"
                  >
                    ⚖️ DNA Break Preset
                  </button>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-4 pt-1">
                <div className="md:col-span-3">
                  <label className="block text-[10px] text-slate-400 font-mono mb-1 font-bold">Event Type</label>
                  <select 
                    value={evtType}
                    onChange={(e: any) => setEvtType(e.target.value)}
                    className="w-full bg-slate-900 border border-slate-800 rounded px-2.5 py-1.5 text-xs text-white focus:outline-none focus:border-[#07C2E3] font-mono"
                  >
                    <option value="world_state">World State</option>
                    <option value="tool">Tool Execution</option>
                    <option value="agent">Agent Communication</option>
                    <option value="memory">Core Memory</option>
                    <option value="knowledge">Knowledge Base</option>
                    <option value="dna">DNA Constitutional</option>
                    <option value="evolution">Ecosystem Evolution</option>
                    <option value="audit">Administrative Audit</option>
                  </select>
                </div>
                <div className="md:col-span-3">
                  <label className="block text-[10px] text-slate-400 font-mono mb-1 font-bold">Priority</label>
                  <select 
                    value={evtPriority}
                    onChange={(e: any) => setEvtPriority(e.target.value)}
                    className="w-full bg-slate-900 border border-slate-800 rounded px-2.5 py-1.5 text-xs text-white focus:outline-none focus:border-[#07C2E3] font-mono"
                  >
                ...
                <div className="md:col-span-3">
                  <label className="block text-[10px] text-slate-400 font-mono mb-1 font-bold">Source Name</label>
                  <input 
                    type="text" 
                    value={evtSource}
                ...
                <div className="md:col-span-3">
                  <label className="block text-[10px] text-slate-400 font-mono mb-1 font-bold">Source Runtime</label>
                  <input 
                    type="text" 
                    value={evtRuntime}
                    onChange={(e) => setEvtRuntime(e.target.value)}
                    className="w-full bg-slate-900 border border-slate-800 rounded px-2.5 py-1.5 text-xs text-white focus:outline-none focus:border-[#07C2E3] font-mono"
                  />
                </div>
              </div>
              ...
              <div>
                <label className="block text-[10px] text-slate-400 font-mono mb-1 font-bold">Serialized Payload (JSON / General Strings)</label>
                <textarea 
                  value={evtPayload}
                  onChange={(e) => setEvtPayload(e.target.value)}
                  rows={2}
                  className="w-full bg-slate-900 border border-slate-800 rounded p-2.5 text-xs text-slate-200 font-mono focus:outline-none focus:border-[#07C2E3] resize-none"
                  placeholder='{"key": "value"}'
                ></textarea>
              </div>

              <div className="flex justify-between items-center pt-1">
                <span className="text-[10px] text-slate-500 font-mono">⚡ Autonomic cascades will automatically fire depending on conditions.</span>
                <button
                  onClick={() => {
                    try {
                      EnterpriseNervousSystemService.getInstance().publishEvent({
                        eventType: evtType,
                        source: evtSource,
                        sourceRuntime: evtRuntime,
                        payload: evtPayload,
                        priority: evtPriority
                      });
                      triggerSuccess('Telemetry Event broadcasted successfully!');
                    } catch (err) {
                      console.error(err);
                    }
                  }}
                  className="px-4 py-2 bg-[#07C2E3] font-bold text-black rounded text-xs hover:bg-[#06B2D0] active:scale-95 transition flex items-center gap-1.5 shadow"
                >
                  📡 Broadcast Event (广播发布)
                </button>
              </div>
            </div>

            {/* 2. Live Event Bus ledger List */}
            <div className="bg-slate-950 border border-slate-850 p-6 rounded-xl shadow-xl space-y-4">
              <div className="flex justify-between items-center">
                <div>
                  <h3 className="font-sans font-extrabold text-sm text-white">Event Log Ledger (事件日志分类账)</h3>
                  <p className="text-[10.5px] text-slate-400">All high speed real-time transaction notifications synchronized globally.</p>
                </div>
                <div className="flex gap-2.5">
                  <button 
                    onClick={() => {
                      dbEngine.nervous_events.purge();
                      triggerSuccess('Event Bus database purged!');
                    }}
                    className="px-2.5 py-1 border border-slate-800 rounded text-slate-400 hover:text-white hover:bg-slate-900 text-[10px] font-mono transition"
                  >
                    🗑️ Clear Bus
                  </button>
                </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs text-slate-300 border-collapse">
                  <thead>
                    <tr className="border-b border-slate-850 text-slate-400 font-mono text-[10px] pb-1">
                      <th className="py-2 pr-2">ID</th>
                      <th className="py-2 px-2">Type</th>
                      <th className="py-2 px-2">Runtime Origin</th>
                      <th className="py-2 px-2">Priority</th>
                      <th className="py-2 px-2">Payload Details</th>
                      <th className="py-2 px-2">Status</th>
                      <th className="py-2 pl-2 text-right">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {dbEngine.nervous_events.getAll().length === 0 ? (
                      <tr>
                        <td colSpan={7} className="text-center py-8 text-slate-500 italic font-mono">
                          No events inside ledger. Deploy a telemetry packet to start.
                        </td>
                      </tr>
                    ) : (
                      dbEngine.nervous_events.getAll().sort((a,b)=>b.created_at.localeCompare(a.created_at)).map(evt => {
                        const typeColor = 
                          evt.event_type === 'world_state' ? 'text-amber-400 bg-amber-400/10' :
                          evt.event_type === 'tool' ? 'text-rose-400 bg-rose-400/10' :
                          evt.event_type === 'dna' ? 'text-red-400 bg-red-400/10' :
                          evt.event_type === 'audit' ? 'text-purple-400 bg-purple-400/10' :
                          'text-[#07C2E3] bg-[#07C2E3]/10';
                          
                        const priorityColor = 
                          evt.priority === 'critical' ? 'text-red-400 font-bold border border-red-900/40 bg-red-950/20' :
                          evt.priority === 'high' ? 'text-amber-400 bg-slate-900' :
                          'text-slate-400 bg-slate-900';

                        return (
                          <tr key={evt.event_id} className="border-b border-slate-900 hover:bg-slate-900/35 transition text-[11px] font-mono">
                            <td className="py-2.5 pr-2 font-bold text-slate-400">[{evt.event_id}]</td>
                            <td className="py-2.5 px-2">
                              <span className={`px-1.5 py-0.5 rounded text-[9px] uppercase font-bold ${typeColor}`}>
                                {evt.event_type}
                              </span>
                            </td>
                            <td className="py-2.5 px-2 text-slate-300">
                              <span className="text-white font-sans">{evt.source}</span>
                              <span className="block text-[9px] text-slate-500 font-mono">{evt.source_runtime}</span>
                            </td>
                            <td className="py-2.5 px-2">
                              <span className={`px-1.5 py-0.2 rounded text-[8.5px] uppercase ${priorityColor}`}>
                                {evt.priority}
                              </span>
                            </td>
                            <td className="py-2.5 px-2 max-w-[150px] truncate text-slate-400" title={evt.payload}>
                              {evt.payload}
                            </td>
                            <td className="py-2.5 px-2">
                              <span className={`font-bold ${evt.status === 'dispatched' ? 'text-green-400' : 'text-amber-400'}`}>
                            ...
                            <td className="py-2.5 pl-2 text-right">
                              <button
                                onClick={() => {
                                  EnterpriseNervousSystemService.getInstance().replayEvent(evt.event_id);
                                  triggerSuccess(`Replaying Event [${evt.event_id}] over Active Sockets!`);
                                }}
                                className="px-2 py-0.5 rounded bg-slate-900 border border-slate-800 text-[9.5px] hover:text-[#07C2E3] text-slate-300 font-bold hover:border-[#07C2E3] transition"
                              >
                                Replay 🔄
                              </button>
                            </td>
            ...
            {/* 3. Subscription Bindings Node Registry */}
            <div className="bg-slate-950 border border-slate-850 p-6 rounded-xl shadow-xl space-y-4">
              <div>
                <h3 className="font-sans font-extrabold text-sm text-white">Event Broadcast Route Registry (事件广播路由表)</h3>
                <p className="text-[10.5px] text-slate-400">Active background agent micro-services subscribed to specific event types.</p>
              </div>

              {/* Grid with preseeded / dynamic subs */}
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {dbEngine.nervous_subscriptions.getAll().map(sub => {
                  return (
                    <div key={sub.subscription_id} className="p-3 bg-slate-900 border border-slate-850 rounded-lg flex justify-between items-center text-xs">
                      <div>
                        <div className="font-sans font-bold text-white text-[12.5px]">{sub.subscriber}</div>
                        <div className="flex gap-2 mt-1.5 text-[9.5px] font-mono">
                          <span className="bg-slate-950 text-[#07C2E3] px-1.5 py-0.5 rounded border border-slate-850">
                            Subcription Type: {sub.event_type.toUpperCase()}
                          </span>
                          <span className="bg-slate-950 text-slate-400 px-1.5 py-0.5 rounded border border-slate-850">
                      ...
                      <div className="text-right">
                        <button
                          onClick={() => {
                            dbEngine.nervous_subscriptions.delete(sub.subscription_id);
                            triggerSuccess('Subscription deleted!');
                          }}
                          className="text-red-400 hover:text-red-500 transition text-[11px] font-mono font-bold"
                        >
                          Unsubscribe ❌
                        </button>
                      </div>
              ...
              {/* Add Custom Subscriber inline */}
              <div className="bg-slate-900/50 border border-slate-850 rounded-lg p-4 space-y-3">
                <span className="text-[10px] text-slate-400 font-mono font-bold block uppercase">Register New Applet / Agent Listener</span>
                <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-end">
                  <div className="md:col-span-4">
                    <label className="block text-[8.5px] text-slate-400 font-mono mb-1 uppercase font-bold">Subscriber Name</label>
                    <input 
                      type="text" 
                      value={subName} 
                      onChange={(e) => setSubName(e.target.value)}
                      className="w-full bg-slate-950 border border-slate-800 rounded px-2.5 py-1 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-[#07C2E3]"
                      placeholder="e.g. Risk Mitigation Thread"
                    />
                  </div>
                  <div className="md:col-span-4">
                    <label className="block text-[8.5px] text-slate-400 font-mono mb-1 uppercase font-bold">Event Type Filter</label>
                    <select 
                      value={subEvtType}
                      onChange={(e: any) => setSubEvtType(e.target.value)}
                      className="w-full bg-slate-950 border border-slate-800 rounded px-2.5 py-1 text-xs text-white focus:outline-none focus:border-[#07C2E3] font-mono"
                    >
                      <option value="all">ALLEVENTS (*)</option>
                  ...
                  <div className="md:col-span-2">
                    <label className="block text-[8.5px] text-slate-400 font-mono mb-1 uppercase font-bold">Priority Weight</label>
                    <input 
                      type="number" 
                      min={1} 
                      max={5} 
                      value={subPriority}
                      onChange={(e) => setSubPriority(parseInt(e.target.value) || 3)}
                      className="w-full bg-slate-950 border border-slate-800 rounded px-2.5 py-1 text-xs text-white focus:outline-none focus:border-[#07C2E3] font-mono"
                    />
                  </div>
                  <div className="md:col-span-2">
                    <button
                      onClick={() => {
                        dbEngine.nervous_subscriptions.create({
                          subscriber: subName,
                          event_type: subEvtType,
                          priority: subPriority,
                          status: 'active'
                        });
                        triggerSuccess(`Registered subscription binding for [${subName}]!`);
                      }}
                      className="w-full py-1 bg-[#07C2E3]/95 text-black font-bold text-xs rounded hover:bg-[#07C2E3] transition active:scale-95"
                    >
                      Subscribe 🔔
                    </button>
                  </div>
          ...
          {/* RIGHT 4 COLUMNS: Agent Message Stream, Direct Dialog & Network Performance metrics */}
          <div className="lg:col-span-4 space-y-6">
            
            {/* 1. Direct Dialog Stream Console */}
            <div className="bg-slate-950 border border-slate-850 p-5 rounded-xl text-white shadow-xl space-y-4 flex flex-col h-[520px]">
              <div>
                <span className="text-[10px] text-[#07C2E3] font-mono font-bold uppercase tracking-wider block">Agent-to-Agent Stream</span>
                <h4 className="font-sans font-bold text-sm text-white mt-0.5">Autonomous Direct Message Log</h4>
                <p className="text-[10px] text-slate-500 leading-relaxed mt-1">Insulated dialogue ledger monitoring collaborative resolution sequences.</p>
              </div>

              {/* Message scroll container */}
              <div className="flex-1 overflow-y-auto space-y-3.5 pr-1 text-[11px] font-sans scrollbar-thin">
                {dbEngine.agent_messages.getAll().length === 0 ? (
                  <div className="text-center text-slate-600 italic py-16 font-mono">No dialogue packets intercepted.</div>
                ) : (
                  dbEngine.agent_messages.getAll().map(msg => {
                    const isSystemOrOther = msg.sender.includes('Sensor') || msg.sender.includes('Intelligence');
                    
                    return (
                      <div key={msg.message_id} className={`p-3 rounded-lg border text-left flex flex-col space-y-1 ${
                        isSystemOrOther 
                        ...
                        <div className="flex justify-between items-center text-[9.5px] text-slate-400 font-mono">
                          <span className="font-bold text-[#07C2E3]">Sender: {msg.sender}</span>
                          <span className="text-slate-500">→ {msg.receiver}</span>
                        </div>
                        <p className="text-white text-[10.5px] leading-relaxed italic mt-1 font-mono">"{msg.content}"</p>
                        <div className="flex justify-between items-center text-[8.5px] text-slate-500 font-mono pt-1">
                          <span>Task context: {msg.task_id}</span>
                          <span className="text-green-400 font-bold uppercase">{msg.status}</span>
                        </div>
                      </div>
                    );
                  })
              ...
              {/* Message input simulate sender drawer */}
              <div className="border-t border-slate-850 pt-3 space-y-2">
                <div className="grid grid-cols-2 gap-2 text-[9px] font-mono">
                  <div>
                    <span className="text-slate-500 block">Sender Agent</span>
                    <select 
                      value={directMsgSender} 
                      onChange={(e) => setDirectMsgSender(e.target.value)}
                      className="w-full bg-slate-900 border border-slate-800 rounded px-1.5 py-0.5 text-white"
                    >
                      <option value="Pricing Coordinator Agent">Pricing Agent</option>
                      <option value="Inventory Hub Agent">Inventory Agent</option>
                      <option value="Tactical Procurement Node">Procurement Node</option>
                      <option value="Governance Compliance Watchdog">Compliance Agent</option>
                    </select>
                  </div>
                  <div>
                    <span className="text-slate-500 block">Recipient Agent</span>
                    <select 
                      value={directMsgReceiver} 
                      onChange={(e) => setDirectMsgReceiver(e.target.value)}
                      className="w-full bg-slate-900 border border-slate-800 rounded px-1.5 py-0.5 text-white"
                    >
                      <option value="Inventory Master Agent">Inventory Agent</option>
                      <option value="Pricing Coordinator Agent">Pricing Agent</option>
                      <option value="Tactical Procurement Node">Procurement Node</option>
                      <option value="Global Order Orchestrator">Orchestrator</option>
                    </select>
                  </div>
                </div>
                <div className="relative flex items-center">
                  <input
                    ...
                    placeholder="Provide message string to broadcast..."
                    className="w-full bg-slate-900 border border-slate-800 rounded px-2.5 py-1.5 text-[11px] font-mono text-white placeholder-slate-700 pr-12 focus:outline-none focus:border-[#07C2E3]"
                  />
                  <button
                    onClick={() => {
                      if (!directMsgContent.trim()) return;
                      EnterpriseNervousSystemService.getInstance().addAgentMessage({
                        taskId: 'task_user_sim',
                        sender: directMsgSender,
                        receiver: directMsgReceiver,
                        messageType: 'instruction',
                        content: directMsgContent
                      });
                      triggerSuccess('Intercepted simulation packet added to message pool!');
                      setDirectMsgContent('');
                    }}
                    className="absolute right-1 text-[#07C2E3] hover:text-white transition px-2 py-0.5 font-bold text-xs"
                  >
                    SEND
                  </button>
                </div>
            ...
            {/* 2. Dispatch Latency and Routing Reliability Index */}
            <div className="bg-slate-950 border border-slate-850 p-5 rounded-xl text-white shadow-xl space-y-4 text-left">
              <div>
                <span className="text-[10px] text-green-400 font-mono font-bold uppercase tracking-wider block font-bold">⚡ Network Diagnostic Indices</span>
                <h4 className="font-sans font-bold text-sm text-white mt-0.5">Telemetry Performance Monitor</h4>
              </div>

              <div className="grid grid-cols-2 gap-3 font-mono">
                <div className="p-2.5 bg-slate-900 rounded border border-slate-850">
                  <span className="text-[9px] text-slate-400 block pb-1">Average Dispatch Latency</span>
                  <span className="text-lg font-bold text-[#07C2E3]">4.82 ms</span>
                </div>
                <div className="p-2.5 bg-slate-900 rounded border border-slate-850">
                  <span className="text-[9px] text-slate-400 block pb-1">Routing Success Rate</span>
                  <span className="text-lg font-bold text-green-400">100.0%</span>
                </div>
              </div>

              {/* Dead letter queue (DLQ) state info */}
              <div className="p-3 bg-slate-900 border border-slate-850 rounded-lg space-y-1 text-xs">
                <div className="flex justify-between items-center text-[10px] font-mono uppercase font-bold text-red-400">
                  <span>⚠️ Dead Letter Queue (DLQ)</span>
                  <span>0 Packets</span>
                </div>
                <p className="text-[10px] text-slate-500 leading-normal">Failed telemetries auto-divert to isolation segment to contain transaction state drifts.</p>
              </div>
            </div>
          </div>
        </div>
      ) : subTab === 'governor' ? (
        /* P0-010 Enterprise Governor Runtime Console */
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 animate-fadeIn text-slate-800">
          {/* Left Column (8 cols): Policies, Decision Log & Audit Trail */}
          <div className="lg:col-span-8 space-y-6">
            
            {/* Class 1: Constitutional Policies Manager */}
            <div className="bg-slate-950 border border-slate-850 p-6 rounded-xl text-white shadow-2xl relative overflow-hidden">
              <div className="absolute top-0 right-0 w-64 h-64 bg-[#07C2E3] opacity-[0.02] rounded-full blur-3xl pointer-events-none" />
              <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
                <div>
                  <span className="text-[10px] text-[#07C2E3] font-mono font-bold uppercase tracking-wider block">CONSTITUTIONAL LAYER</span>
                  <h3 className="font-sans font-bold text-lg text-white">Active Governor Policies (企业运行宪则)</h3>
                </div>
                <span className="bg-[#07C2E3]/10 text-[#07C2E3] text-[10.5px] px-2.5 py-0.5 rounded-full border border-[#07C2E3]/20 font-mono">
                  {dbEngine.governor_policies.getAll().length} Hard Policies Active
                </span>
              </div>

              {/* Inline Policy Creator Form */}
              <div className="p-4 bg-slate-900/60 rounded-lg border border-slate-850 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-5 text-left">
                <div className="md:col-span-2 space-y-1">
                  <label className="text-[10px] text-slate-400 block font-mono">POLICY NAME & STATEMENT</label>
                  <input
                    type="text"
                    value={govPolName}
                    onChange={(e) => setGovPolName(e.target.value)}
                    placeholder="e.g. EU GDPR Customer Opt-out Protection Rule"
                    className="w-full bg-slate-950 border border-slate-800 rounded px-2.5 py-1 text-xs text-white focus:outline-none focus:border-[#07C2E3]"
                  />
                  <input
                    type="text"
                    value={govPolDesc}
                    onChange={(e) => setGovPolDesc(e.target.value)}
                    placeholder="Detailed explanation of limits and trigger penalties..."
                    className="w-full bg-slate-950 border border-slate-800 rounded px-2.5 py-1 text-[10.5px] text-slate-300 placeholder-slate-700 mt-1 focus:outline-none focus:border-[#07C2E3]"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] text-slate-400 block font-mono">POLICY CLASS</label>
                  <select
                    value={govPolType}
                    onChange={(e: any) => setGovPolType(e.target.value)}
                    className="w-full bg-slate-950 border border-slate-800 rounded px-2.5 py-1 text-xs text-white focus:outline-none focus:border-[#07C2E3]"
                  >
                    <option value="financial_limit">Financial Margins Limit</option>
                    <option value="ad_spend">Ad Spend Protection</option>
                    <option value="inventory_risk">Inventory Safeguard</option>
                    <option value="compliance">Compliance Option</option>
                    <option value="permission_escalation">Permission Escalation</option>
                  </select>
                </div>
                <div className="space-y-1 flex flex-col justify-between">
                  <div>
                    <label className="text-[10px] text-slate-400 block font-mono">RISK LEVEL</label>
                    <select
                      value={govPolRisk}
                      onChange={(e: any) => setGovPolRisk(e.target.value)}
                      className="w-full bg-slate-950 border border-slate-800 rounded px-2.5 py-1 text-xs text-white focus:outline-none focus:border-[#07C2E3]"
                    >
                      <option value="low">Low Risk</option>
                      <option value="medium">Medium Risk</option>
                      <option value="high">High Risk</option>
                      <option value="critical">Critical (Veto-Proof)</option>
                    </select>
                  </div>
                  <button
                    onClick={() => {
                      if (!govPolName.trim()) {
                        triggerSuccess('Invalid policy title. Please write a descriptive name.');
                        return;
                      }
                      dbEngine.governor_policies.create({
                        policy_name: govPolName,
                        policy_type: govPolType,
                        description: govPolDesc || 'Constitutional restriction protecting systemic margins',
                        risk_level: govPolRisk,
                        approval_mode: govPolMode,
                        status: 'active'
                      });
                      triggerSuccess('Constitutional policy initialized and locked into database!');
                      setGovPolName('');
                      setGovPolDesc('');
                    }}
                    className="w-full bg-[#07C2E3] text-slate-950 hover:bg-[#06B2D0] font-sans font-bold py-1 px-3 rounded text-xs transition mt-2 cursor-pointer text-center"
                  >
                    + REGISTER RULE
                  </button>
                </div>
                ...
                <table className="w-full text-xs font-mono">
                  <thead>
                    <tr className="border-b border-slate-850 text-slate-400 text-left">
                      <th className="py-2.5 font-sans font-bold">Policy & ID</th>
                      <th className="py-2.5 font-sans font-bold">Category</th>
                      <th className="py-2.5 font-sans font-bold">Threat Level</th>
                      <th className="py-2.5 font-sans font-bold">Auth Mode</th>
                      <th className="py-2.5 font-sans font-bold">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {dbEngine.governor_policies.getAll().map((policy) => (
                      <tr key={policy.policy_id} className="border-b border-slate-900 hover:bg-slate-900/30 text-left">
                        <td className="py-3">
                          <div className="font-sans font-bold text-slate-200">{policy.policy_name}</div>
                          <div className="text-[10px] text-slate-500 leading-tight pr-3 mt-0.5">{policy.description}</div>
                          <span className="text-[9px] bg-slate-900 text-slate-400 px-1 py-0.2 rounded mt-1 inline-block">{policy.policy_id}</span>
                        </td>
                        <td className="py-3">
                          <span className="text-[10px] font-bold text-slate-300 uppercase bg-slate-900 border border-slate-800 px-1.5 py-0.5 rounded">
                            {policy.policy_type.replace('_', ' ')}
                          </span>
                        </td>
                        <td className="py-3">
                          <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded uppercase ${
                            policy.risk_level === 'critical' ? 'bg-red-950 text-rose-400 border border-red-900' :
                            policy.risk_level === 'high' ? 'bg-orange-950 text-orange-400 border border-orange-900' :
                            policy.risk_level === 'medium' ? 'bg-amber-950 text-amber-400 border border-amber-900' :
                        ...
                        <td className="py-3 space-x-2">
                          <button
                            onClick={() => {
                              ...
                              dbEngine.governor_policies.update(policy.policy_id, { status: newStatus });
                              triggerSuccess(`Policy status modified to: ${newStatus.toUpperCase()}`);
                            }}
                            className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                              policy.status === 'active' ? 'bg-teal-950 text-teal-300 border border-teal-900' : 'bg-slate-800 text-slate-400 border border-slate-700'
                            }`}
                          >
                            {policy.status === 'active' ? 'ACTIVE' : 'MUTED'}
                          </button>
                          <button
                            onClick={() => {
                              dbEngine.governor_policies.delete(policy.policy_id);
                              triggerSuccess('Constitutional policy deleted from persistence block.');
                            }}
                            className="bg-red-950 text-rose-400 hover:bg-rose-900/30 px-1.5 py-0.5 rounded border border-red-900 text-[10px]"
                          >
                            DELETE
                          </button>
                        </td>
            ...
            {/* Class 2: Governor Decisions Ledger */}
            <div className="bg-slate-950 border border-slate-850 p-6 rounded-xl text-white shadow-2xl text-left">
              <div className="flex justify-between items-center mb-4">
                <div>
                  <span className="text-[10px] text-[#07C2E3] font-mono font-bold uppercase tracking-wider block">INTELLIGENT DECISION LEDGER / ARBITER QUEUE</span>
                  <h3 className="font-sans font-bold text-lg text-white">Systemic Ruling Records (最终裁决账本)</h3>
                </div>
                <button
                  onClick={() => {
                    dbEngine.governor_decisions.purge();
                    triggerSuccess('Governor decisions ledger purged.');
                  }}
                  className="bg-slate-900 border border-slate-850 text-slate-400 hover:text-white font-mono text-[10px] px-2.5 py-1 rounded"
                >
                  PURGE DECISIONS
                </button>
              </div>
                ...
                <table className="w-full text-xs font-mono">
                  <thead>
                    <tr className="border-b border-slate-850 text-slate-400">
                      <th className="py-2.5 text-left">Decision & ID</th>
                      <th className="py-2.5 text-left">Caller Node</th>
                      <th className="py-2.5 text-left">Veto Output</th>
                      <th className="py-2.5 text-right">Risk Score / Confidence</th>
                    </tr>
                  </thead>
                  <tbody>
                    {dbEngine.governor_decisions.getAll().map((dec) => (
                      <tr key={dec.decision_id} className="border-b border-slate-900 hover:bg-slate-900/30">
                        <td className="py-3">
                          <div className="font-bold text-slate-200">{dec.reason}</div>
                          <div className="text-[10px] text-slate-500 mt-0.5">
                            ID: <span className="text-slate-400">{dec.decision_id}</span> | Task Ref: <span className="text-slate-400">{dec.task_id}</span> | Time: <span className="text-slate-400">{new Date(dec.created_at).toLocaleTimeString()}</span>
                          </div>
                        </td>
                        ...
                        <td className="py-3">
                          <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded uppercase ${
                            dec.decision === 'approved' ? 'bg-emerald-950 text-emerald-400 border border-emerald-900' :
                            dec.decision === 'rejected' ? 'bg-rose-950 text-rose-400 border border-rose-900' :
                            dec.decision === 'escalated' ? 'bg-amber-950 text-amber-400 border border-amber-900' :
                        ...
                        <td className="py-3 text-right">
                          <div className="font-bold text-[#07C2E3]">{dec.risk_score} / 100</div>
                          <div className="text-[10px] text-slate-500">Confidence: {(dec.confidence * 100).toFixed(0)}%</div>
                        </td>
                      </tr>
                    ))}
                  </tbody>
            ...
            {/* Class 3: Realtime Governor State Audit Trail */}
            <div className="bg-slate-950 border border-slate-850 p-6 rounded-xl text-white shadow-2xl text-left">
              <div className="flex justify-between items-center mb-4">
                <div>
                  <span className="text-[10px] text-[#07C2E3] font-mono font-bold uppercase tracking-wider block font-bold">GOVERNOR AUDIT COMPLIANCE DECK</span>
                  <h3 className="font-sans font-bold text-lg text-white">Constitutional Execution Audits (安全合规审计轨迹)</h3>
                </div>
                <button
                  onClick={() => {
                    dbEngine.governor_audit_logs.purge();
                    triggerSuccess('Governor audits purged.');
                  }}
                  className="bg-slate-900 border border-slate-850 text-slate-400 hover:text-white font-mono text-[10px] px-2.5 py-1 rounded"
                >
                  PURGE AUDITS
              ...
              <div className="space-y-3 font-mono">
                {dbEngine.governor_audit_logs.getAll().map((aud) => (
                  <div key={aud.audit_id} className="p-3 bg-slate-900 border border-slate-850 rounded-lg space-y-2 text-xs">
                    <div className="flex justify-between items-center flex-wrap gap-2">
                      <div className="flex items-center gap-2">
                        <span className="font-bold text-slate-200">Audit {aud.audit_id}</span>
                        <span className="text-[10px] bg-slate-950 border border-slate-800 text-[#07C2E3] px-2 py-0.5 rounded">
                          Action: {aud.action.toUpperCase()}
                        </span>
                      </div>
                      <div className="text-[10px] text-slate-500">
                        Op: <span className="text-slate-300 font-bold">{aud.operator}</span> | {new Date(aud.created_at).toLocaleString()}
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-[10.5px]">
                      <div className="p-2 bg-slate-950 rounded border border-slate-900">
                        <span className="text-[9px] text-slate-500 block">BEFORE STATE / TRANS-CONTEXT:</span>
                        <pre className="text-amber-400 overflow-x-auto whitespace-pre-wrap leading-tight mt-1">{aud.before_state}</pre>
                      </div>
                      <div className="p-2 bg-slate-950 rounded border border-slate-900">
                        <span className="text-[9px] text-slate-500 block">AFTER STATE / RULING PATHWAY:</span>
                        <pre className="text-green-400 overflow-x-auto whitespace-pre-wrap leading-tight mt-1">{aud.after_state}</pre>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          {/* Right Column (4 cols): Control Console, Lockdown, and Simulator */}
          <div className="lg:col-span-4 space-y-6 text-left">
            
            {/* Widget 1: Circuit Breaker Safety Console */}
            <div className="bg-slate-950 border border-slate-850 p-5 rounded-xl text-white shadow-xl space-y-4">
              <div>
                <span className="text-[10px] text-rose-400 font-mono font-bold uppercase tracking-wider block font-bold">🚨 AUTOMATIC DISRUPTIVE RELIABILITY</span>
                <h4 className="font-sans font-bold text-sm text-white mt-0.5">Systemic Circuit Breaker (系统熔断保险丝)</h4>
              </div>
              ...
              <div className={`p-4 rounded-lg flex items-center gap-3 border ${
                ...
                <div className={`w-3.5 h-3.5 rounded-full ${
                  EnterpriseGovernorService.getInstance().isCircuitBreakerTripped() ? 'bg-rose-500' : 'bg-green-500'
                }`} />
                <div>
                  <div className="text-xs font-bold uppercase">
                    Breaker State: {EnterpriseGovernorService.getInstance().isCircuitBreakerTripped() ? 'TRIPPED (QUARANTINED)' : 'CLOSED (NORMAL RUNTIME)'}
                  </div>
                  {EnterpriseGovernorService.getInstance().isCircuitBreakerTripped() && (
                    <div className="text-[10px] text-slate-400 leading-tight mt-0.5">
                      Reason: {EnterpriseGovernorService.getInstance().getCircuitBreakerReason()}
                    </div>
                  )}
                </div>
              </div>

              {/* Trip Control Trigger input */}
              <div className="space-y-1.5 text-xs font-mono">
                <label className="text-[10px] text-slate-400 block pb-0.5">MANUAL WATCHDOG TRIP REASON</label>
                <input
                  type="text"
                  value={breakerReasonInput}
                  onChange={(e) => setBreakerReasonInput(e.target.value)}
                  placeholder="e.g. Memory pollution threat or infinite run loops"
                  className="w-full bg-slate-900 border border-slate-800 rounded px-2.5 py-1 text-xs text-white uppercase focus:outline-none focus:border-[#07C2E3]"
                />
              </div>
              ...
              <div className="flex gap-2 font-mono text-xs">
                <button
                  onClick={() => {
                    if (!breakerReasonInput.trim()) return;
                    EnterpriseGovernorService.getInstance().triggerCircuitBreaker(breakerReasonInput);
                    triggerSuccess('CRITICAL WARNING: Manual circuit breaker tripped! Autonomous pipelines frozen.');
                  }}
                  className="w-1/2 bg-red-950 text-rose-400 border border-red-900/60 hover:bg-rose-900/30 font-bold py-1 px-2 rounded tracking-tight py-1.5 transition text-center text-[11px]"
                >
                  TRIP BREAKER
                </button>
                <button
                  onClick={() => {
                    ...
                    triggerSuccess('Systemic circuit reset. Normal multi-agent interactions resumed.');
                  }}
                  className="w-1/2 bg-slate-900 border border-slate-800 text-slate-300 hover:text-white font-bold py-1 px-2 rounded py-1.5 transition text-center text-[11px]"
                >
                  RESET BREAKER
                </button>
              </div>
            ...
            {/* Widget 2: Emergency Manual Takeover Hub */}
            <div className="bg-slate-950 border border-slate-850 p-5 rounded-xl text-white shadow-xl space-y-4">
              <div>
                <span className="text-[10px] text-[#07C2E3] font-mono font-bold uppercase tracking-wider block font-bold">⚙️ EXTREME STRUCTURAL CONTROL</span>
                <h4 className="font-sans font-bold text-sm text-white mt-0.5">Emergency Enterprise Takeover (紧急人工接管)</h4>
              </div>
              ...
              <div className={`p-4 rounded-lg flex items-center justify-between border ${
                EnterpriseGovernorService.getInstance().isSystemLockedDown()
                  ? 'bg-rose-950/50 border-rose-900 text-rose-400 font-bold text-xs'
                  : 'bg-slate-900 border-slate-850 text-slate-400 text-xs'
              }`}>
                <span>🔒 Runtime Lockdown:</span>
                <span className="uppercase">{EnterpriseGovernorService.getInstance().isSystemLockedDown() ? 'LOCKED (MANUAL CONTROL)' : 'RUNNING (AUTONOMOUS)'}</span>
              </div>

              <p className="text-[10px] font-mono text-slate-500 leading-normal">
                Triggering the administrative takeover enforces absolute execution locks. Sub-atomic agents, execution tools, and transaction adapters are halted instantly.
              </p>
              ...
              <div className="grid grid-cols-2 gap-2 text-xs font-mono">
                <button
                  onClick={() => {
                    ...
                    triggerSuccess('SYSTEM SECURED: Global structural runtime locked. AI workers suspended.');
                  }}
                  className="w-full bg-rose-900 text-white hover:bg-rose-800 font-bold py-2 rounded text-center transition cursor-pointer text-[10.5px]"
                >
                  🔒 LOCK DOWN SYSTEM
                </button>
                <button
                  onClick={() => {
                    ...
                    triggerSuccess('SYSTEM OPENED: Structural runtime unlocked. Background worker threads resume.');
                  }}
                  className="w-full bg-emerald-900 text-white hover:bg-emerald-800 font-bold py-2 rounded text-center transition cursor-pointer text-[10.5px]"
                >
                  🔓 RESUME RUNTIME
                </button>
              </div>
            ...
            {/* Widget 3: Transaction Threat Sandbox Simulator */}
            <div className="bg-slate-950 border border-slate-850 p-5 rounded-xl text-white shadow-xl space-y-4">
              <div>
                <span className="text-[10px] text-[#07C2E3] font-mono font-bold uppercase tracking-wider block font-bold">🧪 SANDBOX TEST VETOES</span>
                <h4 className="font-sans font-bold text-sm text-[#07C2E3] mt-0.5">Proposal Security Simulator (运行评测沙盒)</h4>
              </div>

              <div className="space-y-3 font-mono text-xs text-left">
                <div className="grid grid-cols-2 gap-2">
                  <div className="space-y-1">
                    <label className="text-[9px] text-slate-400">CALLER AGENT</label>
                    <input
                      type="text"
                      value={simSource}
                      onChange={(e) => setSimSource(e.target.value)}
                      className="w-full bg-slate-900 border border-slate-850 rounded px-2 py-1 text-slate-100"
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[9px] text-slate-400">TASK ID</label>
                    <input
                      type="text"
                      value={simTaskId}
                ...
                <div className="space-y-1">
                  <label className="text-[9px] text-slate-400">PROPOSAL ACTION TYPE</label>
                  <select
                    value={simActionType}
                    onChange={(e: any) => setSimActionType(e.target.value)}
                    className="w-full bg-slate-900 border border-slate-850 rounded px-2 py-1 text-slate-100"
                  >
                    <option value="price_change">Price change (e.g., Margin evaluation)</option>
                    <option value="stock_purchase">Stock Purchase (Stock level & Supplier costs)</option>
                    <option value="ad_campaign">Advertising Campaign (Spend spike check)</option>
                    <option value="system_config">System Configuration Action</option>
                    <option value="compliance_check">Opt-out Compliance Veto</option>
                  </select>
                </div>

                <div className="space-y-1">
                  <label className="text-[9px] text-slate-400">
                    {simActionType === 'price_change' ? 'PROPOSED CONTEXT MARGIN ( % )' :
                     simActionType === 'stock_purchase' ? 'ESTIMATED EXPENDITURE BUDGET ( USD )' :
                     simActionType === 'ad_campaign' ? 'BUDGET GROWTH PERCENTAGE ( % )' :
                     'REPRESENTATIVE PARAM VALUE'}
                  </label>
                  <input
                    type="number"
                    value={simValue}
                    onChange={(e: any) => setSimValue(parseFloat(e.target.value) || 0)}
                    className="w-full bg-slate-900 border border-slate-850 rounded px-2 py-1 text-slate-100 text-left"
                  />
                </div>
                ...
                <button
                  onClick={() => {
                    ...
                    if (simActionType === 'price_change') {
                      payload.margin = simValue;
                      payload.discount_pct = 100 - simValue;
                    } else if (simActionType === 'stock_purchase') {
                      payload.estimated_cost = simValue;
                    } else if (simActionType === 'ad_campaign') {
                      payload.spike_pct = simValue;
                      payload.daily_budget = simValue * 15;
                    } else {
                      payload.parameter = simValue;
                    }

                    const result = EnterpriseGovernorService.getInstance().evaluateProposal({
                      taskId: simTaskId,
                      source: simSource,
                    ...
                    let outcomeAlert = `SANDBOX RESULTS: Proposal classified. Decision: ${result.decision.toUpperCase()}. `;
                    if (result.decision === 'approved') {
                      outcomeAlert += `Safe transaction confirmed! Calculated risk: ${result.risk_score} points.`;
                    } else if (result.decision === 'rejected') {
                      outcomeAlert += `VETO PENALTY ENFORCED: Operation blocked. Context breached constitutional policies.`;
                    } else if (result.decision === 'escalated') {
                      outcomeAlert += `ELEVATED SIGNATURE: Transferred to Multi-Signature super-authorization queue.`;
                    } else {
                      outcomeAlert += `BLOCKED: Target layers lock down.`;
                    }

                    triggerSuccess(outcomeAlert);
                  }}
                  className="w-full bg-[#07C2E3] text-slate-950 font-sans font-bold py-2 rounded shadow hover:bg-[#06B2D0] cursor-pointer text-center text-[11px]"
                >
