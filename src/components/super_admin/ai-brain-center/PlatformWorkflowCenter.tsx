...
export default function PlatformWorkflowCenter({ addLog }: PlatformWorkflowCenterProps) {
  ...
  const loadPresetWorkflow = (presetName: string) => {
    ...
    } else if (presetName === 'botble_sync') {
      setVisualNodes([
        { id: 'p_b1', type: 'trigger', status: 'idle', title: 'Botble CMS Webhook Event', details: 'Triggers when Laravel 12 emits low stock alert, raw product uploads, or inactive VIP notifications.' },
        { id: 'p_b2', type: 'ai_decision', status: 'idle', title: 'Multi-Agent Board Consensus', details: 'Oliver (WMS), Fiona (Pricing) and Leo (Catalog) debate inventory reorders and copy revisions.' },
        { id: 'p_b3', type: 'condition', status: 'idle', title: 'SaaS Constitution & Governance Audit', details: 'Confirms that the simulated action keeps gross margins safely above the 35% baseline requirement.' },
        { id: 'p_b4', type: 'action', status: 'idle', title: 'Transmit Laravel API State Override', details: 'Pushes authenticated CRM coupon codes or updated product stock structures directly to Botble DB.' }
      ]);
      setSelectedNodeId('p_b1');
      addLog('Workflow Engine 2.0', 'Laravel & Botble Sync Preset Loaded', 'Successfully loaded Botble CRM Multi-Agent Autonomy preset.', 'success');
    } else {
      setVisualNodes([
    ...
    <div id="tab-visual-workflows-panel" className="space-y-6 animate-fadeIn text-slate-800">
      ...
      <div className="bg-slate-900 text-white rounded-2xl p-6 shadow-md border border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div className="flex flex-wrap gap-2 shrink-0">
          ...
          <button
            onClick={() => loadPresetWorkflow('botble_sync')}
            className="bg-zinc-950 hover:bg-black text-[#07C2E3] font-bold text-[10px] px-3 py-2 rounded-xl border border-[#07C2E3]/50 transition-colors cursor-pointer shadow-md"
          >
            🔌 Load Preset: Laravel & Botble Sync Flow
          </button>
          <button
